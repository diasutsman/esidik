...
{
    $curl = curl_init($url);
    curl_setopt_array($curl, array(CURLOPT_HEADER => true, CURLOPT_NOBODY => true, CURLOPT_RETURNTRANSFER => true, CURLOPT_FOLLOWLOCATION => true, CURLOPT_URL => $url));
...
function getSesi()
{
    ...
    return ($nses==0) ? (5*60*1000) : ($nses* 60*1000);
...
function apipostdata($url,$withttps=false,$array){
