...
$url_pag =  "'".site_url("shiftkhusus/pagging/0")."'";
$domId ="'#list-data'";
...
<div class="row">
		<div class="ibox float-e-margins">
			<div class="ibox-title">
				<h5>Daftar Shift Kerja Khusus</h5>
	...
	$(document).ready(function(){
		...
		$("#btnAdd").click(function(e){
			...
			if (isNew==0) {
				...
				newRow.insertCell(13).innerHTML = '<input type="input" name="colour_shift" class="colorpict"/>';
				...
				cell =  newRow.insertCell(14);
				cell.innerHTML = '<select  name="state" class="newdata"><option value="0">Tidak Aktif</option><option value="1">Aktif</option></select>';
				cell.style.width= "50px";

				newRow.insertCell(15).innerHTML = '<a class="btn btn-xs btn-primary" onclick="doSave()" data-id="0" title="Save"><i class="fa fa-save"></i></a><a class="btn btn-xs btn-danger" onclick="doCancel();" data-id="0" title="Remove"><i class="fa fa-undo"></i></a>';
				isNew++;
		...
		$('#btncari').click(function(){
			load_url('<?php echo site_url('shiftkhusus/pagging/0') ?>',"#list-data");
		});
		...
		if(cek > 0){
			bootbox.confirm("Anda yakin menghapus data yang dipilih?", function(result) {
				if(result) {
					...
					$.ajax({
						url     : '<?php echo site_url('shiftkhusus/hapus');?>',
						dataType: 'json',
						type    : 'POST',
						data    : { 'id' : cek_del},
	...
	function doSave()
	{
			bootbox.confirm("Anda yakin menyimpan data tersebut?", function(result) {
				if(result) {
					...
					$.ajax({
						url     : '<?php echo site_url('shiftkhusus/simpanbaru');?>',
						dataType: 'json',
						type    : 'POST',
						data    : data,
						success : function(data){
