...
class Home extends MX_Controller {
	...
	function index_list()
	{
	    ...
	    $unitKerjaUser= $this->session->userdata('s_dept');
        ...
        if($this->setting_model->tegdis()) {
            ...
            if($this->setting_model->dpoison($expl[0])==$stepone) {
                ...
                $dataarray = array (
                    'devallow'		=> $dal[1],
                    'maxlog'		=> 'Unlimited',
                    'exdate'		=> $this->setting_model->dpoison($expl[2])=='Unlimited'?'Unlimited':date('d F Y', $this->setting_model->dpoison($expl[2])),
                    'serverid'		=> $this->setting_model->dpoison($expl[0])
                );
            } else {
                $etad = $this->setting_model->tegetad() + (30*86400);
                $dataarray = array (
                    'devallow'		=> 50,
                    'maxlog'		=> 500,
                    'exdate'		=> date('d F Y', $etad),
                    'serverid'		=> $stepone
                );
            }
        } else {
