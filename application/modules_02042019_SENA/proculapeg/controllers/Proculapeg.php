...
class Proculapeg extends MX_Controller {
    ...
    function pegawai()
    {
        ...
        foreach($datas as $rpeg) {
            ...
            //hari libur
            /*$holar = array();
            $holiday = $this->utils->cekholiday($rpeg->tmulai, $rpeg->takhir);
            ...
            for($x=0;$x<=$range;$x++) {
                ...
                if ($stsapprov=='Y') {
                    ...
                    if ($rsl->num_rows() > 0) {
                        ...
                        createLog("Status pegawai " . $nip . " " . $atdate . " " . $attcode, "Sukses");
                    }
                }
            }
    ...
    private function sendtoula($luserid){
        ...
        $urlpost='https://ulapeg.setjen.kemendagri.go.id/restulapeg/index.php/api/approval_cuti';
        foreach($datas as $rpeg) {
            $arrintd =
    ...
    {
        /*$datas = $this->mdl_ulapeg->gettuserid(5);
        $urlpost='https://ulapeg.setjen.kemendagri.go.id/restulapeg/index.php/api/approval_cuti';
        foreach($datas as $rpeg) {
            $arrintd =
                'kode_usulan='.$rpeg->kode_usulan."&".
            ...
            $buffer = apipostdata($urlpost,true,$arrintd);
            var_dump($buffer);

        }*/

        $datas = $this->mdl_ulapeg->gettuserid(1);
        $urlpost='https://ropeg.setjen.kemendagri.go.id/restsimpeg/index.php/ulapeg/insert_cuti';
        foreach($datas as $rpeg) {
    ...
    {
        $datas = $this->mdl_ulapeg->gettuserid(1);
        $urlpost='http://192.168.193.172:8090/esidik/api/presensi/ulapeg';
        foreach($datas as $rpeg) {
            $arrintd =
                'token=359b8b18726aa37907a3c01613f9ebc6&'.
                'kode_usulan='.$rpeg->kode_usulan."&".
                'nip='.$rpeg->nip."&".
                'jenis_cuti='.$rpeg->jenis_cuti."&".
                'jum_hari='.$rpeg->jum_hari."&".
            ...
            $buffer = apipostdata($urlpost,false,$arrintd);
            var_dump($buffer);

        }

        /*$datas = $this->mdl_ulapeg->gettuserid(1);
        $urlpost='https://ropeg.setjen.kemendagri.go.id/api2018/insert_cuti';
        foreach($datas as $rpeg) {
            if ($rpeg->status_persetujuan==='Y') {
                $arrintd = array(
                    'kode_usulan' => $rpeg->kode_usulan,
