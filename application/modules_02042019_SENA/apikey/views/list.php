...
$url_pag =  site_url("apikey/pagging/0");
...
<div class="table-responsive">
    ...
    <table class="table table-striped table-bordered">
        ...
        <tr>
            ...
            <th onClick="load_url('<?php echo $url_pag?>','key','<?php echo (isset($order)?(($order=='key')?$sorting:'sorting'):'sorting_desc')?>')" ><div class="<?php echo (isset($order)?(($order=='key')?$typeorder:'sorting'):'sorting_desc')?>">Key</div></th>
            <th onClick="load_url('<?php echo $url_pag?>','note','<?php echo (isset($order)?(($order=='note')?$sorting:'sorting'):'sorting')?>')" ><div class="<?php echo (isset($order)?(($order=='note')?$typeorder:'sorting'):'sorting')?>">Keterangan</div></th>
            <th onClick="load_url('<?php echo $url_pag?>','expired','<?php echo (isset($order)?(($order=='expired')?$sorting:'sorting'):'sorting')?>')" ><div class="<?php echo (isset($order)?(($order=='expired')?$typeorder:'sorting'):'sorting')?>">Kadaluarsa</div></th>
            <th></th>
        </tr>
        </thead>
        <tbody>
            ...
            <div class="col-sm-9 m-b-xs">
                <div id="tabel_data_length" class="dataTables_length">
                    <div class="form-inline">
                        ...
                        $js = 'id="limit_display" class="input-sm" onChange="load_url(\''.$url_pag.'\',\''.(isset($order)?$order:'id').'\',\''.(isset($typeorder)?$typeorder:'sorting_desc').'\');" name="tabel_data_length" size="1" ';
...
<script>
    $(function(){
        ...
        $('.editrow').click(function(){
            ...
            $.getJSON('<?php echo site_url('apikey/edit')?>/'+xno,function() {})
    ...
    function createpaging()
    {
        $("#pagering").find("a").each(function(i){
            ...
            $(this).bind('click', function(){
                load_url(thisHref,"<?php echo (isset($order)?$order:'id')?>","<?php echo (isset($typeorder)?$typeorder:'sorting_desc')?>");
                return false;
            });
        });
    }
