...
<div class="row">
		<div class="ibox float-e-margins">
			<div class="ibox-title">
				<h5>Daftar Akses Level Pengguna</h5>
				<div class="ibox-tools">
                    <a class="btn btn-sm btn-info" href="<?php echo site_url("usrlevel")?>">Level Pengguna</a>
			...
			<div class="ibox-content">
				<div class="row">
					<div class="col-sm-5 m-b-xs">
						<a  id="btnDeleteRow" href="#" class="btn btn-sm btn-danger" onClick="doDelete();" ><i class="fa fa-minus"></i> Hapus</a> <a id="btnAdd" class="btn btn-sm btn-success"><i class="fa fa-plus"></i> Tambah</a>
					</div>
...
<script>
	$(function(){
		...
		$('#btncari').click(function(){
			load_url('<?php echo site_url('usraction/pagging/0') ?>','<?php echo (isset($order)?$order:'user_level_id')?>','<?php echo (isset($typeorder)?$typeorder:'sorting')?>');
		});
		...
		if(cek > 0){
			bootbox.confirm("Anda yakin menghapus data yang dipilih?", function(result) {
				if(result) {
					...
					$.ajax({
						url     : '<?php echo site_url('usraction/hapus');?>',
						dataType: 'json',
						type    : 'POST',
						data    : { 'id' : cek_del},
