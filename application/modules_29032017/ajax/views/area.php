...
<?php
	$sql = $this->db->query("SELECT * FROM personnel_area WHERE parent_id is null ORDER BY areaid ASC");
	...
	?>
		<div class="tree-folder" style="display: block;">
			<div class="tree-folder-header hd0">
				<i id="fd<?php echo $row->areaid?>" class="icon-plus" onClick="TreeMenu('<?php echo $row->areaid?>',1)"></i>
				<div class="tree-folder-name fn0" onClick="PilihData('<?php echo $row->areaid?>','<?php echo $row->areaname?>')"><?php echo $row->areaname?></div>
				</div>
			<div id="tr<?php echo $row->areaid?>" class="tree-folder-content" style="display: none;">
		
			</div>
			<div id="load<?php echo $row->areaid?>" class="tree-loader" style="display: none;">
...
function TreeMenu(v,lvl)
{
	...
	if(namaClass=='icon-plus'){
		...
		$.ajax({
			url     : '<?php echo site_url('ajax/child_area')?>/'+v+'/'+lvl,
			dataType: 'html',
			type    : 'POST',
