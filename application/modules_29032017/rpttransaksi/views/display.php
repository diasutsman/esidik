				...
				<div class="form-group">
					...
					<div class="col-sm-7 form-inline" id="data_5">
						<div class="input-daterange input-group" id="datepicker">
							<input type="text" class="input-sm form-control" name="start" id="start" value="<?php echo date("01-m-Y", strtotime("-1 months", strtotime(date("Y-m-d"))))?>"/>
							<span class="input-group-addon">s/d</span>
							<input type="text" class="input-sm form-control" name="end" id="end" value="<?php echo date("t-m-Y", strtotime("-1 months", strtotime(date("Y-m-d"))))?>" />
						</div>
					</div>
				</div>
				<div class="form-group">
					<label class="control-label col-md-2 col-sm-3 col-xs-12" for="jnspal">Dasar Laporan</label>
...
$(function(){
	...
	$('#btncaridata').click(function(){
		load_url('<?php echo site_url('rpttransaksi/pagging/0') ?>',"#list-data");
	});
	...
	$('#btnproses').click(function(){
		...
		$('<form action="<?php echo site_url('rpttransaksi/view');?>" method="POST" target="_blank" style="display:none">' +
