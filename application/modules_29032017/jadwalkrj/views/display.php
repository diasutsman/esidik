...
<script>
	$(function(){
		...
		$('#btncari').click(function(){
			...
			$.ajax({
				...
				data:{"lmt":par4,"cari":par5,"org":par6,"stspeg":par7,"start":par8,"end":par9},
	...
	function saveForm() {
		...
		$.ajax({
			url: '<?php echo site_url('jadwalkrj/statusdata');?>/'+$('#pil').val(),
				...
				if ( (response.data != "[]") || (response.data != "[[]]")) {
					...
					for (var i = 0; i < $datv.length; i++) {
						if ($datv[i] != 'undefined') {
							var str = $("#" + $datv[i].id).data('rec');
							if (str.indexOf("#") < 0) {
								if (( $('#pil').val()==1) || ($('#pil').val()==2 )) {
									$("#" + $datv[i].id).attr('data-rec', str + $datv[i].sts);
								}
							} else {
								if (( $('#pil').val()==3)) {
									var res = str.split("#");
