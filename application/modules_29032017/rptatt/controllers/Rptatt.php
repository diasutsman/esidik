    ...
    public function view()
    {
        ...
        if ($pilrpt == '1') {
            ...
            for ($x = 0; $x <= $range; $x++) {
                ...
                if ($yo == 1) {
                    $querytemp = $this->report_model->getattbydateorg($areaid, $tanggal, $orgid,$stspeg);
                } else if ($yo == 2) {
                    $querytemp = $this->report_model->getattbydateuser($tanggal, $userar,$stspeg);
                } else if ($yo == 3) {
                    $querytemp = $this->report_model->getattbydate($areaid, $tanggal,$stspeg);
                }
            ...
            foreach ($dataallay as $queqe) {
                ...
                $datafoot = array('totallate' => $this->report_model->itungan($totallate), 'totalearly' => $this->report_model->itungan($totalearly),
                    'totalotbef' => $this->report_model->itungan($totalotbef), 'totalotaf' => $this->report_model->itungan($totalotaf), 'total' => $this->report_model->itungan($total));
