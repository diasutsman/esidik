...
class Rpttunjangan extends MX_Controller {
    ...
    function view1()
    {
        ...
        foreach($rslt as $queq) {
            ...
            //echo $queq->userid." ".$unkir." ".$unkirplt."<br>";
            //die();
        ...
        foreach($dataallay as $queqe) {
            ...
            foreach($querytemp->result() as $que) {
                ...
                */ 
                ...
                if ($que->date_shift2!="" && $queqe['payable'] !=0  && $tubel==0 && $isAlpha==0)
                {			
	...
	function view3()
    {
        ...
        foreach($dataallay as $queqe) {
            ...
            foreach($querytemp->result() as $que)
            {
                ...
                {
                    $totalpersenkurang = $totalpersenkurang + $krglate;
                    $tunj += (($krglate /100) * ($tunjangan[date('mY', strtotime($que->date_shift))] * $ttlmsk[date('mY', strtotime($que->date_shift))]));
                    ...
                    if ($check_in == null) {
						...
						//$y = round($t);
						$jumTdkAbsen += ceil(round((($krglate / 100) * ($tunjangan[date('mY', strtotime($que->date_shift))] * $ttlmsk[date('mY', strtotime($que->date_shift))])),2));
                        $jumTdkAbsenPlt += ceil(round((($krglate / 100) * ($tunjanganplt[date('mY', strtotime($que->date_shift))] * $ttlmsk[date('mY', strtotime($que->date_shift))])),2));
                    } else {
                        $jumlate += ceil(round((($krglate / 100) * ($tunjangan[date('mY', strtotime($que->date_shift))] * $ttlmsk[date('mY', strtotime($que->date_shift))])),2));
                        $jumlatePlt += ceil(round((($krglate / 100) * ($tunjanganplt[date('mY', strtotime($que->date_shift))] * $ttlmsk[date('mY', strtotime($que->date_shift))])),2));
                    }

                    $totallate += ceil(round(((($krglate /100) * ($tunjangan[date('mY', strtotime($que->date_shift))] * $ttlmsk[date('mY', strtotime($que->date_shift))]))),2));
                    $totallatePlt += ceil(round((($krglate /100) * ($tunjanganplt[date('mY', strtotime($que->date_shift))] * $ttlmsk[date('mY', strtotime($que->date_shift))])),2));
                }
                ...
                if($krgearly > 0 && $que->attendance!='NWK' && $que->attendance!='NWDS' && $que->workinholiday!=1 ) {
                    ...
                    $tunj +=  (($krgearly /100) * ($tunjangan[date('mY', strtotime($que->date_shift))] * $ttlmsk[date('mY', strtotime($que->date_shift))]));
                    $tunjplt += (($krgearly /100) * ($tunjanganplt[date('mY', strtotime($que->date_shift))] * $ttlmsk[date('mY', strtotime($que->date_shift))]));

                    if ($check_out  == null) {
                        $jumTdkAbsen += ceil(round((($krgearly / 100) * ($tunjangan[date('mY', strtotime($que->date_shift))] * $ttlmsk[date('mY', strtotime($que->date_shift))])),2));
                        $jumTdkAbsenPlt += ceil(round((($krgearly / 100) * ($tunjanganplt[date('mY', strtotime($que->date_shift))] * $ttlmsk[date('mY', strtotime($que->date_shift))])),2));
                    } else {
                        $jumearly += ceil(round((($krgearly / 100) * ($tunjangan[date('mY', strtotime($que->date_shift))] * $ttlmsk[date('mY', strtotime($que->date_shift))])),2));
                        $jumearlyPlt += ceil(round((($krgearly / 100) * ($tunjanganplt[date('mY', strtotime($que->date_shift))] * $ttlmsk[date('mY', strtotime($que->date_shift))])),2));
                    }
                }
                ...
                if($krgalpa != 0 && $que->workinholiday!=1) {
                    ...
                    $totalalpa++;
					$isAlpha=1;
					$tunj += (($krgalpa /100) * ($tunjangan[date('mY', strtotime($que->date_shift))] * $ttlmsk[date('mY', strtotime($que->date_shift))]));
                    $tunjplt += (($krgalpa /100) * ($tunjanganplt[date('mY', strtotime($que->date_shift))] * $ttlmsk[date('mY', strtotime($que->date_shift))]));
                    //alpha
                    $jumlalpa += ceil(round((($krgalpa /100) * ($tunjangan[date('mY', strtotime($que->date_shift))] * $ttlmsk[date('mY', strtotime($que->date_shift))])),2));
                    $jumlalpaPlt += ceil(round((($krgalpa /100) * ($tunjanganplt[date('mY', strtotime($que->date_shift))] * $ttlmsk[date('mY', strtotime($que->date_shift))])),2));
                }
                ...
                if($krgstatus != 0  && $queqe['payable']!=0) {
                    ...
                    $tunj += (($krgstatus /100) * ($tunjangan[date('mY', strtotime($que->date_shift))] * $ttlmsk[date('mY', strtotime($que->date_shift))]));
                    $tunjplt += (($krgstatus /100) * ($tunjanganplt[date('mY', strtotime($que->date_shift))] * $ttlmsk[date('mY', strtotime($que->date_shift))]));

                    $jumijin += ceil(round((($krgstatus /100) * ($tunjangan[date('mY', strtotime($que->date_shift))] * $ttlmsk[date('mY', strtotime($que->date_shift))])),2));
                    $jumijinPlt += ceil(round((($krgstatus /100) * ($tunjanganplt[date('mY', strtotime($que->date_shift))] * $ttlmsk[date('mY', strtotime($que->date_shift))])),2));
                }
                ...
                */ 
                        ...
                        if($pa != '' or $pa != null) //cek jadwal
                        {
                            ...
                            if ($cekinupcara < $jwdlawal  || $cekinupcara > $jwdlakhir)
                            {
                                ...
                                if ($ispotong==1){
                                    ...
                                    $jumUpacara += ceil(round((($nkurang / 100) * ($tunjangan[date('mY', strtotime($que->date_shift))] * $ttlmsk[date('mY', strtotime($que->date_shift))])),2));
								    $jumUpacaraPlt += ceil(round((($nkurang / 100) * ($tunjanganplt[date('mY', strtotime($que->date_shift))] * $ttlmsk[date('mY', strtotime($que->date_shift))])),2));
                                }
                            } 
                        }
                    //print_r($this->db->last_query());
                ...
                if(($que->attendance=='AB_18' || $que->attendance=='AB_19' || $que->attendance=='AB_12' ) && $queqe['payable']!=0) {
                    ...
                    if (isset($pkrg[$que->attendance])) {
                        ...
                        $tunjplt += (($krgalpa / 100) * ($tunjanganplt[date('mY', strtotime($que->date_shift))] * $ttlmsk[date('mY', strtotime($que->date_shift))]));
                        //alpha
                        $jumlalpa += ceil(round((($krgalpa / 100) * ($tunjanganplt[date('mY', strtotime($que->date_shift))] * $ttlmsk[date('mY', strtotime($que->date_shift))])),2));
                        $jumlalpaPlt += ceil(round((($krgalpa / 100) * ($tunjanganplt[date('mY', strtotime($que->date_shift))] * $ttlmsk[date('mY', strtotime($que->date_shift))])),2));
                    }
                }
        ...
        if($excelid==1) {
            header("Content-type: application/x-msdownload");
            header("Content-Disposition: attachment; filename=lappotongankeuangantunjangankinerja.xls");
            //echo "$datavw";
