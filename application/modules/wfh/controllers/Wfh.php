...
class Wfh extends MX_Controller {
	...
	function Wfh(){
		...
		$this->load->model('wfh_model','wfh');
        $this->load->model('pegawai/pegawai_model','pegawai');

        if( !$this->auth->is_logged_in() ){
            redirect('login', 'refresh');
        }
        $this->aAkses = akses("Wfh", $this->session->userdata('s_access'));
    ...
    function index_list()
    {
        ...
        $SQLcari .=" and a.status= '29' ";
        $SQLcari .= " ORDER BY a.checktime DESC ";
        $query = $this->wfh->getDaftar(1,10,$offset,null,$SQLcari);
        $jum_data = $this->wfh->getDaftar(0,null,null,null,$SQLcari);
        $this_url = site_url('wfh/pagging/');
    ...
    public function pagging($page=0)
    {
		...
		$SQLcari .=" and (date(a.checktime) between '".$mulai."' and '".$akhir."' and a.status= '29' )";
        ...
        $query = $this->wfh->getDaftar(1,$limited,$offset,null,$SQLcari);
        $jum_data = $this->wfh->getDaftar(0,null,null,null,$SQLcari);
        $this_url = site_url("wfh/pagging");
