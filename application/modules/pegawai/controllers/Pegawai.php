...
class Pegawai extends MX_Controller {
    ...
    public function form($noId=null)
    {
		...
		$query_peg = $this->db_simpeg->query("SELECT p.*, COALESCE(e.neselon,'') AS eselon FROM peg_identpeg AS p LEFT JOIN peg_jakhir AS j ON p.nip = j.nip LEFT JOIN ref_eselon AS e ON j.keselon = e.keselon WHERE p.nip='".str_replace(" ","",$datar["badgenumber"])."'");
		$peg = $query_peg->row();
		$datar["no_rekening"] = ($peg != null) ? $peg->no_rekening : "";
		$datar["eselon"] = ($peg != null) ? str_replace(" ", "", $peg->eselon) : "";		
       $data["field"] = $datar;
    ...
    public function save()
    {
        ...
        $dataIn['tmtkelasjabatan'] =  dmyToymd($this->input->post('tmtkelasjabatan'));

        $tunjprof = $this->input->post('tunjanganprofesi');
        $tunjprofdate = $this->input->post('tmtprofesi');
        $kelasjabatan = $this->input->post('kelasjabatan');
            ...
            if(!$update){
                ...
                if ($tunjprof != null && $tunjprofdate != null) {
                    $q = $this->db->select('userid')
                        ...
                        ->where(array('userid' => $userid, 'tunjprofdate' => $tunjprofdate,'tunjanganprofesi' => ($tunjprof != '' ? $tunjprof : null)))->get();
                    if ($q->num_rows() == 0) {
                        $history = array(
                            'userid' => $userid,
                ...
                //kelas jabatan
                if ($dataIn['tmtkelasjabatan'] != null) {
                    $q = $this->db->select('userid')
                        ->from('userinfohistory')
                        ->where(array('userid' => $userid,
                            'tmtjabatan' => $dataIn['tmtkelasjabatan']))->get();
                    if ($q->num_rows() == 0) {
                        $history = array(
                            'userid' => $userid,
                            'tmtjabatan' => $dataIn['tmtkelasjabatan'],
                            'kelas' => $kelasjabatan
                        );
                        $this->db->insert('userinfohistory', $history);
                    } else {
                        $this->db->delete("userinfohistory",
                            array('userid' => $userid, 'tmtjabatan' => $tunjprofdate));
                ...
                if ($dataIn['tmt_plt'] != null)
                {
                    ...
                    $history = array(
                        ...
                        'userid' => $userid,
                        'tmt' => $dataIn['tmt_plt'],
                        'deptid' => $dataIn['plt_deptid'],
                        'jbtn' => $dataIn['plt_jbtn'],
                        'sk' => $dataIn['plt_sk'],
                        'kelasjabatan' => $dataIn['plt_kelasjabatan'],
                        'plt_eselon' => $dataIn['plt_eselon'],
                        'eselon' => $dataIn['eselon'],
                        'nilai'=>$mastertunj[$dataIn['plt_kelasjabatan']],
                    );
                    $this->db->insert('userinfohistory', $history);
                ...
                if ($adaJson) {
                    ...
                    if ($npil==0) {
                        ...
                        foreach ($aListPeg as $itemdata) {
							...
							$dataup["nskjabat_ps"] = $itemdata["nskjabat_ps"];
							$dataup["tskjabat_ps"] = $itemdata["tskjabat_ps"];
							$dataup["status_ps"] = $itemdata["status_ps"];
                            ...
                            $dataup["golru"] = $golru;
                            $dataup["kelasjabatan"] = $itemdata["peringkat2"];

                            $dataup["jftstatus"] = $itemdata["status_pegawai"];
							$dataup["jftdate"] = $itemdata["tgl_peg"];
                            ...
                            //end history
                            $dataup["tmtkelasjabatan"] = strlen(trim($tmt))==0?$tmtkelas:dmyToymd($tmt);
                            ...
                            if ($itemdata["status_plt"] == "1") { //jika status PLT
								...
								$dataplt['jenis']			= 'PLT';
								$dataplt['userid']			= $itemdata["nip"];
								$dataplt['tmt']				= $itemdata["tmtjab_plt"];
								$dataplt['deptid']			= $kdUnker;
								$dataplt['eselon']			= $dataup["eselon"];
								$dataplt['kelasjabatan']	= $itemdata["peringkat_plt"];
								$dataplt['plt_eselon']		= $dataup["plt_eselon"];
								$dataplt['jbtn']			= $itemdata["njab_plt"];
								$dataplt['sk']				= $itemdata["sk_plt"];
								$xss_data = $this->security->xss_clean($dataplt);
								$this->db->insert('tukinhistory', $xss_data);
                            } else {
                                ...
                                if ($itemdata["status_plh"] == "1") { //Jika status PLH
									...
									$dataplt['jenis']			= 'PLH';
									$dataplt['userid']			= $itemdata["nip"];
									$dataplt['tmt']				= $itemdata["tmtjab_plh"];
									$dataplt['deptid']			= $kdUnker;
									$dataplt['eselon']			= $dataup["eselon"];
									$dataplt['kelasjabatan']	= $itemdata["peringkat_plh"];
									$dataplt['plt_eselon']		= $dataup["plt_eselon"];
									$dataplt['jbtn']			= $itemdata["njab_plh"];
									$dataplt['sk']				= $itemdata["sk_plh"];
									$xss_data = $this->security->xss_clean($dataplt);
									$this->db->insert('tukinhistory', $xss_data);
                                } else {
                            ...
                            if ($tmtkelas != null) {
                                $history = array(
                                    'userid' => trim($itemdata["nip"]),
                                    'tmtjabatan' => strlen(trim($tmt))==0?$tmtkelas:dmyToymd($tmt),
                                    'kelas' => $kelas != '' ? $kelas : 0
                                );
                                ...
                                $history = array(
                                    'userid' => trim($itemdata["nip"]),
                                    'tmtjabatan' => strlen(trim($tmt))==0?$tmtjabatan:dmyToymd($tmt),
                                    'kelas' => $kelas != '' ? $kelas : 0
                                );
    ...
    public function Area($noId=null)
    {
		...
		if($aksespegawai == '1'){
			...
			$this->template->load('template','akses',$data);
		}
    }
        ...
        if (isset($dataSimpeg['datapegawai'])) {
            ...
            foreach ($aListPeg as $itemdata) {
                ...
                /*if (!empty($itemdata['peringkat_definitif']) &&
                    ...
                    $data['ListArray'][$i]['kelasjabatan'] = $itemdata['peringkat2'];
            ...
            foreach ($aListPeg as $itemdata) {
                if ($itemdata['nip']===$nip)
                {
                    ...
                    $data['field']=array(
                        ...
                        "jftstatus"=>$itemdata["status_pegawai"],
                        'kelasjabatan' => $itemdata["peringkat2"],
                        'golru' => $golru,
