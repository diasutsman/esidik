			...
			<div class="ibox-content">
				...
				<div class="row">
                    ...
                    <div class="col-sm-1 m-b-xs">
                        <div class="btn-group">
                            <?php 
                            //var_dump($this->session->userdata('s_access'));
                            ...
                            <ul class="dropdown-menu" role="menu">
                                ...
                                <li><a href="#" id="btn4" onclick="doCopyData()">Daftarkan FP Ke Mesin</a>
                                </li>
                                <li><a href="#" id="btn5" onclick="doSinkronData()">Sinkronisasi dengan SIMPEG</a>
                                </li>
                                <li><a href="#" id="btn6" onclick="doUpdateUID()">Ubah USER ID/NIP</a>
                                </li>
                                <!--<li><a href="<?php /*echo site_url("pegawai/listsimpeg")*/?>" id="btn7" target="_blank">Pegawai Belum Ter-register</a>
                                </li>-->
                                <li><a href="#" id="btn1" onclick="doDelete()">Hapus Pegawai</a>
                                </li>
                                <li><a href="#" id="btn7" onclick="doMoveData()">Pindah data presensi</a>
                                </li>
...
<form class="form-horizontal form-label-left" name="copyForm" id="changeUserForm" method="post">
    <div class="popup-wrapper" id="popup4">
        <div class="popup-container">
            <div class="modal-header">
                <span type="button" class="close" data-dismiss="modal" onClick="hidePopup('#popup4');">&times;</span>
                <h3>Mengganti USER ID di Mesin / Mengganti NIP</h3>
            </div>
            <div class="modal-body" style="height: 100px;">
                <div class="form-group">
                    <label class="control-label col-md-3 col-sm-3 col-xs-12" for="newUID">User ID Baru</label>
                    <div class="col-md-5 col-sm-5 col-xs-12">
                        <input type="text" id="newUID" name="newUID" placeholder="User ID Baru" class="input-sm form-control" maxlength="30">
                        <input type="hidden" name="olduseid" id="olduseid"/>
                    </div>
                </div>
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-sm btn-success" data-dismiss="modal" onClick="saveUpdateUserForm();">
                    Simpan
                </button>
            </div>
        </div>
...
<form class="form-horizontal form-label-left" name="moveDataForm" id="moveDataUserForm" method="post">
    <div class="popup-wrapper" id="popup5">
        <div class="popup-container">
            <div class="modal-header">
                <span type="button" class="close" data-dismiss="modal" onClick="hidePopup('#popup5');">&times;</span>
                <h3>Transfer data presensi pegawai</h3>
            </div>
            <div class="modal-body" style="height: 100px;">
                <div class="form-group">
                    <label class="control-label col-md-3 col-sm-3 col-xs-12" for="newUID">Target Pegawai</label>
                    <div class="col-md-5 col-sm-5 col-xs-12">
                        <input type="text" id="newUID" name="newUID" placeholder="NIP Pegawai" class="input-sm form-control" maxlength="30">
                        <input type="hidden" name="olduseid" id="olduseidm"/>
                    </div>
                </div>
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-sm btn-success" data-dismiss="modal" id="prcmovedata" onClick="saveMoveDataForm();">
                    Simpan
                </button>
            </div>
        </div>
...
<script>
    ...
    function doDeleteFP() {
        ...
        if (cek > 0) {
            bootbox.confirm("Anda yakin menghapus FP pegawai dari mesin?<br><br><i>Note: Akan menghapus FP berdasarkan Area</i>", function (result) {
                if (result) {
    ...
    function doUpdateUID()
    {
        var cek = $(".selected:checked").length;
        if (cek == 1) {
            showPopup('#popup4');
        } else{
            bootbox.alert("Harap pilih data / Pilih Satu Data saja..!");
        }
    }
    ...
    function saveUpdateUserForm()
    {
        var cek_del = $('input[name=cek_del]:checked').map(function () {
            return $(this).val();
        }).get();
        $("#olduseid").val(cek_del);
        $.ajax({
            url: '<?php echo site_url('pegawai/updateuid');?>',
            dataType: 'json',
            type: 'POST',
    ...
    function BootboxContent(varMsg) {
        var frm_str = varMsg+'<form class="bootbox-form">'
            +'<select class="bootbox-input bootbox-input-select form-control" id="pilsink"><option value="0">Sesuai kondisi saat ini...</option><option value="1">Hanya pegawai yang tidak ada di aplikasi</option></select>'
            +'<div class="form-group">'
            + '<label for="date">TMT Kelas (isi jika TMT Kelas Jabatan berbeda dgn TMT Jabatan)</label>'
            + '<input id="date" class="date md-2 form-control input-sm" size="16" placeholder="dd-mm-yy" type="text" placeholder="TMT" >'
            + '</div>'
            +'</form>';
    ...
    function doSinkronData() {
        var cek = $(".selected:checked").length;
        var cMsg= "Anda yakin men-sikronisasi <b>semua data pegawai</b> dengan data SIMPEG ?<br>Waktu proses membutuhkan waktu cukup lama.. ";
        var cekData="All";
        if (cek > 0) {
            cekData = $('input[name=cek_del]:checked').map(function () {
                return $(this).val();
            }).get();
            cMsg="Anda yakin men-sikronisasi <b>data pegawai yang di pilih</b> dengan data SIMPEG ?<br>Waktu proses membutuhkan waktu cukup lama.. ";
        }
                ...
                ok: {
                    ...
                    callback: function(){
                        $.ajax({
                            ...
                            dataType: 'json',
                            type: 'POST',
                            data: {'id': cekData,'pil':$("#pilsink").val(),'tmt':$("#date").val()},
                            success: function (data) {
                                bootbox.alert(data.msg);
                            }
                        });
    ...
    function doDelete() {
        var cek = $(".selected:checked").length;
        if (cek > 0) {
            bootbox.confirm("Anda yakin menghapus Pegawai dari mesin?<br><br><i>Note: Akan menghapus data Finger Print</i>", function (result) {
                if (result) {
                    var cek_del = $('input[name=cek_del]:checked').map(function () {
                        return $(this).val();
                    }).get();
                    ...
                    $.ajax({
                        url: '<?php echo site_url('pegawai/deldata');?>',
                        dataType: 'json',
                        type: 'POST',
                        data: {'id': cek_del},
                        success: function (data) {
    ...
    function saveMoveDataForm()
    {
        var cek_del = $('input[name=cek_del]:checked').map(function () {
            return $(this).val();
        }).get();
        $("#olduseidm").val(cek_del);
        $.ajax({
            url: '<?php echo site_url('pegawai/cekdata');?>',
            dataType: 'json',
            type: 'POST',
            data: $("#moveDataUserForm").serialize(),
            success: function (data) {
                if (data.status != "error") {
                    bootbox.confirm("Anda yakin memindahkan data pegawai.. ?"+data.msg, function (result) {
                        if (result) {
                            $.ajax({
                                url: '<?php echo site_url('pegawai/movedata');?>',
                                dataType: 'json',
                                type: 'POST',
                                data: $("#moveDataUserForm").serialize(),
                                success: function (data) {
