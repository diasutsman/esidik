...
class Prosess extends MX_Controller
{
    ...
    function index_list()
    {
        $this->session->set_userdata('menu', '21');
        $uri_segment = 3;
        $offset = 0;
        ...
        $this_url = site_url('prosess/pagging/');
    ...
    public function save()
    {
        ...
        $startdate = dmyToymd($this->input->post('start'));
        $enddate = dmyToymd($this->input->post('end'));

        if (array_key_exists("uid",$this->input->post()))
        {
            $this->actionbyuserid($id, $startdate, $enddate, $stspeg);
        } else {
            $this->actionbyOrg(($orgid == "" ? "undefined" : $orgid), $startdate, $enddate, $stspeg);
        }
    }
        ...
        function actionbyuserid($userid = null, $startdate = null, $enddate = null, $stspeg = null)
        {
            $useraidi = $userid;//explode(',', substr($this->input->get('userid'),0,-1));
            ...
            if ($bukatutup || $this->session->userdata('s_access') == 1) {
                ...
                $shift = $this->process_model->getshiftuserid($useraidi, $datestart, $dateend, $stspeg);
                $arrai = array();
                ...
                $rsl=$this->db->get('process');
                $result =$rsl->result();
                foreach($result as $row) {
                    $this->db->where('id', $row->id);
                    $query=$this->db->get("process");
                    $datas = $query->row_array();
                    log_history("delete","process",$datas);

                    if (isset($datas)) {
                        createLog("Menghapus proses data " . $datas["userid"] . " " . $datas["date_shift"], "Sukses");
                    }
                }
                ...
                $user = $this->process_model->getuserbyuser($useraidi);
                foreach ($user->result() as $userya) $deptid[$userya->userid] = $userya->deptid;
                    ...
                    for ($x = 0; $x <= $range; $x++) {
                        ...
                        if (isset($arrai[$userdata][strtotime($tanggal)])) {
                            ...
                            if ($arrai[$userdata][strtotime($tanggal)]['codeshift'] == 'PMD' || $arrai[$userdata][strtotime($tanggal)]['codeshift'] == 'S1C') {
                                ...
                                if ($arrai[$userdata][strtotime($tanggal)]['break_out'] < $arrai[$userdata][strtotime($tanggal)]['check_in']) $tanggalbo = date('Y-m-d', strtotime($tanggal) + 86400);
                                if ($arrai[$userdata][strtotime($tanggal)]['break_in'] < $arrai[$userdata][strtotime($tanggal)]['check_in']) $tanggalbi = date('Y-m-d', strtotime($tanggal) + 86400);
                                if ($arrai[$userdata][strtotime($tanggal)]['end_break'] < $arrai[$userdata][strtotime($tanggal)]['check_in']) $tanggaleb = date('Y-m-d', strtotime($tanggal) + 86400);
                                ...
                                if ($arrai[$userdata][strtotime($tanggal)]['check_out'] < $arrai[$userdata][strtotime($tanggal)]['check_in']) $tanggalco = date('Y-m-d', strtotime($tanggal) + 86400);
                                if ($arrai[$userdata][strtotime($tanggal)]['end_check_out'] < $arrai[$userdata][strtotime($tanggal)]['check_in']) $tanggaleco = date('Y-m-d', strtotime($tanggal) + 86400);
                            }
                            ...
                            createLog("Membuat proses data " . $userdata . " " . date('Y-m-d', strtotime($tanggal)), "Sukses");
                            ...
                            /* $savesimpeg = array(
                                ...
                                'datang'			=> $timein,
                                'pulang'			=> $timeout,
                                'islibur'			=> $workinholiday,
                                'ijin'				=> $attend,
                                'keterangan'		=> $attend
                            );
                            //$secdb = $this->load->database('second',true);
                            $secdb->insert('tr_kehadiran', $savesimpeg); */
                        } else {
        ...
        function actionbyOrg($orgidp = "undefined", $startdate = null, $enddate = null, $stspeg = null)
        {
