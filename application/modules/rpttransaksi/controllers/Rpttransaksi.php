...
class Rpttransaksi extends MX_Controller {
    ...
    public function view()
    {
        ...
        if($orgid!='undefined')
            ...
            $orgid = count($this->session->userdata('s_dept'))>0 ? $this->pegawai->deptonall(convertToArray($this->session->userdata('s_dept'))) : array();
        }

        //print_r($this->session->userdata('s_area'));
        ...
        if($pilrpt=='1') {
            ...
            for($x=0;$x<=$range;$x++) {
                ...
                if($yo==1) {
                    $querytemp = $this->report_model->gettranslogbydateorg($tanggal, $orgid, $areaid,$stspeg,$jnspeg,$moderpt);
                } else if($yo==2) {
                    $querytemp = $this->report_model->gettranslogbydateuser($tanggal, $userar,$stspeg,$jnspeg,$moderpt);

                } else if($yo==3) {
                    $querytemp = $this->report_model->gettranslogbydate($tanggal, $areaid,$stspeg,$jnspeg,$moderpt);
                }
            ...
            foreach($dataallay as $queqe) {
                $querytemp = $this->report_model->gettranslog($datestart, $datestop, $queqe['userid'],$moderpt);
