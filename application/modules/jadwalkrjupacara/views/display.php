...
<div class="row">
	<div class="ibox float-e-margins">
		<div class="ibox-title">
			<h5>Daftar Jadwal Upacara Pegawai</h5>
			<div class="ibox-tools">
				...
				<ul class="dropdown-menu dropdown-user">
					...
					<li><a href="#" id="btn2">Pembatalan Upacara</a>
					</li> 
					<li><a href="#" id="btn4">Hapus Pembatalan Upacara</a>
					</li>
					<li><a href="#" id="btn3">Hapus Jadwal Upacara</a>
					</li>
				</ul>
				<a class="collapse-link">
					<i class="fa fa-chevron-up"></i>
...
<script>
	...
	$(function(){
		...
		$('#btncari').click(function(){
			...
			$.ajax({
				method:"post",
				url: "<?php echo site_url('jadwalkrjupacara/buatjadwal') ?>",
	...
	function saveForm() {
		...
		$.ajax({
			url: '<?php echo site_url('jadwalkrjupacara/statusdata');?>/'+pildata,
                    ...
                    if ((response.data != "[]") || (response.data != "[[]]")) {
                        ...
                        for (var i = 0; i < $datv.length; i++) {
                            if ($datv[i] != 'undefined') {
								...
								$("#" + $datv[i].id).attr('data-rec', $datv[i].sts);
								//$("#" + $datv[i].id).attr('style', "");
								$("#" + $datv[i].id).html($datv[i].sts);
    ...
    /* function showPil1()
    {
        ...
        var url ="<?php echo site_url('jadwalkrjupacara/form1')?>/"+$('#tglpil').val()+"/"+$('#tglpil2').val();
    ...
    */
    ...
    {
        $('#pil').val("2");
        $('#title').html("<span>Pembatalan Upacara</span>");
        if ($('#useriddata').val()=="")
        {
            bootbox.alert("Pilih dahulu datanya..");
            return;
        }
        var url ="<?php echo site_url('jadwalkrjupacara/form2')?>/"+$('#tglpil').val()+"/"+$('#tglpil2').val();
        $("#input_data").html('<div style="position:absolute;left:0;right:0;top:50%;"><center></center></div>').load(url);
        showPopup('#popup');
    } 
    ...
    {
        $('#pil').val("3");
        $('#title').html("<span>Penghapusan Status Upacara</span>");
        ...
        var url ="<?php echo site_url('jadwalkrjupacara/form3')?>/"+$('#tglpil').val()+"/"+$('#tglpil2').val();
        $("#input_data").html('<div style="position:absolute;left:0;right:0;top:50%;"><center></center></div>').load(url);
        showPopup('#popup');
    }
    ...
    {
        $('#pil').val("4");
        $('#title').html("<span>Penghapusan Pembatalan Upacara</span>");
        var cek_del  = $('input[name=cek_del]:checked').map(function(){
            return $(this).val();
        }).get();
