...
class Smsmasking_model extends CI_Model {
    ...
    public function getNamaSanksi($value=null)
    {
        $namasanksi='';
        if ($value !=null){
            $this->db->where($value." between min_nilai and max_nilai ",null,false);
            $this->db->limit(1);
            $queryr=$this->db->get('data_sanksi');
            if($queryr->num_rows()==1)
            {
                $namasanksi=$queryr->row()->nama_sanksi;
            }
        }
        return $namasanksi;
