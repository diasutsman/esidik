			...
			<div class="row form-horizontal">
				<div class="form-group">
					...
					<div class="col-sm-7">
						...
						$selected = array("1","2");
						if ( isset($filter_stspeg) && is_array($filter_stspeg))
						{
							$selected=$filter_stspeg;
						}
				...
				<div class="form-group">
					...
					<div class="col-sm-7">
						...
						$selected = array("1","2");
						if ( isset($filter_jnspeg) && is_array($filter_jnspeg))
						{
							$selected=$filter_jnspeg;
						}
				...
				<div class="form-group">
					...
					<div class="col-sm-7">
						<div class="unker input-group">
							<input type="text" class="input-sm form-control" readonly name="cari_unker" id="cari_unker" value="<?php echo $this->utils->carideptnamebykode($filter_org)?>" placeholder="Unit Kerja ...">
							<input type="hidden" name="unit_search" id="unit_search" value="<?php echo $filter_org?>">
				...
				<div class="form-group">
					...
					<div class="col-sm-7">
						<input type="text" placeholder="Pencarian" class="input-sm form-control" id="caridata" value="<?php echo $filter_cari?>">
					</div>
				</div>
				<div class="form-group">
					...
					<div class="col-sm-7 form-inline" id="data_5">
						<div class="input-daterange input-group" id="datepicker">
							<input type="text" class="input-sm form-control" name="start" id="start" value="<?php echo empty($filter_tgl1)? date("01-m-Y"):$filter_tgl1 ?>"/>
							<span class="input-group-addon">s/d</span>
							<input type="text" class="input-sm form-control" name="end" id="end" value="<?php echo empty($filter_tgl2) ? date("t-m-Y", strtotime(date("Y-m-d"))): $filter_tgl2 ?>" />
						</div>
					</div>
				</div>
				<div class="form-group">
...
$(function(){
	...
	$('#btnproses').click(function(){
		...
		$.ajax({
                    dataType: 'json',
                    type    : 'POST',
                    url: '<?php echo site_url('smsmasking/process') ?>',
                    data:{"org":par6,"stspeg":par7,"jnspeg":par10,"userid":cek_del,"start":par8,"end":par9},
                    success: function(data){
						$("#btnproses").prop('disabled', false);
                        $("#btnproses").html('<i class="fa fa-inbox"></i> Send SMS');
