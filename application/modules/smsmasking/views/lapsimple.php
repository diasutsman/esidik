...
<table width="100%" border="1" class="tblnoborder" cellspacing="0" cellpadding="0">
    ...
    <th width="400">Atasan</th>
    <th width="180">Jumlah Ketidakhadiran</th>
    <th width="250">Sanksi</th>
    <th width="300">Pengelola</th>
    </tr>
    <?php 
    $no=1;
    foreach($querycok->result() as $que) {
        ...
        if (count($atasan)>0)
        {
            ...
            if (count($atasan2)>0)
            {
                $nip=$atasan2['nip'];
                $nama=$atasan2['nama'];
                $jabatan=$atasan2['title'];
            }
            }
        }

        $namapengelola=$this->utils->getNamaUser($this->session->userdata('s_username'));
        //if ($value>=5)
        $compa = $this->report_model->getcompany();
        if ($value >= intval($compa->row()->batas_alpa_sms))
        {
            $namasanksi=$this->mdl_sms->getNamaSanksi($value);
    ...
    <tr>
        ...
        <td align="left" class="text" valign="top"><?php echo $nama."<br>NIP. ".$nip."<br>".$jabatan;?></td>
        <td align="center" class="num" valign="top"><?php echo $value?></td>
        <td align="left" class="num" valign="top"><?php echo $namasanksi?></td>
        <td align="left" class="num" valign="top"><?php echo $namapengelola?></td>
	</tr>
    <?php 
        $no++;
        }
