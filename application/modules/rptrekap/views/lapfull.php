...
<?php
	...
	$headcol .= '<th width="30" class="HDR">HDR</th>
				...
				<th width="30" class="TB">MNG</th>
                <th width="30" class="TB">DPK</th>
                <th width="30" class="TB">UPC</th>
				</tr>';
	...
	foreach($querycok->result() as $que) {
        ...
        {
            //echo $que->date_shift2." => ".$que->date_in2."<br>";
                if (isset($que->date_in2))
                {
                    $datarekap[$que->userid][$que->date_shift.'-1']='UPC';
                }
        }
		//echo substr($que->attendance,0,2) ." ".$que->date_shift." - ".$datarekap[$que->userid][$que->date_shift]."<br>";
	}
	$tampilanReport .= '<tr align="center"><td colspan="'.(count($arr_days)+15).'" align="left" ><h4> Unit Kerja: '.$nama_dept.'</h4></td></tr>';
	...
	{				
		...
		$P = 0; $A = 0; $OFF = 0; $L = 0; $S = 0; $C = 0; $DL = 0; $PDK = 0; $TB = 0;$M = 0;$DPK = 0;$upacara=0;
		for($i=0;$i<count($arr_days);$i++){		
			if(isset($datarekap[$dataatt->userid][$arr_days[$i]])) {
				$tampilanReport .= '<td align="center" class="'.$datarekap[$dataatt->userid][$arr_days[$i]].'">'.$datarekap[$dataatt->userid][$arr_days[$i]];
                ...
                if (isset($datarekap[$dataatt->userid][$arr_days[$i].'-1'])){
                    if ($datarekap[$dataatt->userid][$arr_days[$i].'-1']=='UPC') {
                        $upacara++;
                        $tampilanReport .= '<br>UPC';
                    }
                }
                $tampilanReport .= '</td>';
			} else {
    ...
    <tr>
        <td width="<?php echo $pdfid==1?'75%':'50%' ?>">
            <table>
                ...
                <tr>
                    <td width="50" align="center" class="TB">MNG</td>
                    <td width="450">MENINGGAL DUNIA</td>
                </tr>
                <tr>
                    <td width="50" align="center" class="TB">DPK</td>
                    <td width="450">DPK</td>
                </tr>
                <tr>
                    <td width="50" align="center" class="TB">UPC</td>
                    <td width="450">UPACARA</td>
                </tr>
            </table>
        </td>
