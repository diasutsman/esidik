    ...
    public function view1()
    {
        ...
        if($selall != 'true'){
            foreach($dataallay as $queqe) {
                ...
                $totallate=0; $totalearly=0; $totalot=0; $total=0; $workinholiday=0; $attendance=0; $wfh=0; $absence=0; $off=0; $alpha=0;$editcome=0;$edithome=0;$workday=0;
                $holonnwds=0;$totalholiday  = 0;$totalHolWork=0; $atname=0;
                $totalUpacara=0;
                $jmlUpacaraYa=0;
                foreach($querytemp->result() as $que) {
                    ...
                    if(isset($atar[$que->attendance]) AND $que->status == 0) {
                        $attendance++;
                        ...
                        !array_key_exists($que->attendance, $bbar) AND $que->status == 0
                    ) {
                        $attendance++;
                    }

                     if(isset($atar[$que->attendance]) AND $que->status == 1) {
                        $wfh++;
                        if (isset($aten[$queqe['userid']][$que->attendance]))
                            $aten[$queqe['userid']][$que->attendance]++;
                    } else if ((isset($que->check_in) || isset($que->check_out )) &&
                        $que->workinholiday!=2 && $que->workinholiday!=1 &&!array_key_exists($que->attendance, $atar)  &&
                        !array_key_exists($que->attendance, $bbar) AND $que->status == 1
                    ) {
                        $wfh++;
                    }
                    ...
                    if(isset($atar[$que->attendance])) {
                        $atname++;
                        if (isset($atar[$queqe['userid']][$que->attendance]))
                            $aten[$queqe['userid']][$que->attendance]++;
                    }
                ...
                $dataarray[] = array (
                    ...
                    'totalkehadiran'=> $attendance!=0?$attendance+$wfh:0,
                    'wfh'           => $wfh!=0?$wfh:0,
                    'aten'			=> $aten,
