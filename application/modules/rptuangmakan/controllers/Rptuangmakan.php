...
class Rptuangmakan extends MX_Controller {
    private $aAkses;
	function Rptuangmakan(){
		parent::__construct();
        ...
        $this->load->model('uangmakan_model',"uangmakan");
        $this->load->model('procuangmakan/procdata_model',"procmakan");

        if( !$this->auth->is_logged_in() ){
            redirect('login', 'refresh');
        }
        $this->aAkses = akses("Rptuangmakan", $this->session->userdata('s_access'));
    }
    ...
    function index_list()
    {
        ...
        $this_url = site_url('rptuangmakan/pagging/');
    ...
    public function pagging($page=0)
    {
        ...
        /*$startdate = date("Y-m-d",strtotime($start."-01"));
        $enddate = date("Y-m-t", strtotime($start."-01"));*/
        ...
        $query = $this->pegawai->getDaftar(1,$limited,$offset,null,$SQLcari,null,"Golru DESC, kelasjabatan DESC");
        $datapeg=array();
        foreach($query->result() as $row)
        {
            $kdGol = konversiGolongan($row->golru);
            $umk = $this->uangmakan->ref_uangmakan($kdGol);
            $jml = $this->procmakan->getJmlHari($row->userid,$startdate,$enddate);
			$jmlwfh = $this->procmakan->getJmlHariWFH($row->userid,$startdate,$enddate);
            $wfh = $jmlwfh->row();
			$jmlkotor = $jml *$umk["nominal"];
            $pajak = $jmlkotor*($umk["pajak"]/100);
            $jmlBersih = $jmlkotor-$pajak;
            $datapeg[]=array("userid"=>$row->userid,"name"=>$row->name,"golru"=>$row->golru,
                "tarif"=>$umk["nominal"],"pajak"=>$pajak,"hadir"=>$jml,
                "kotor"=>$jmlkotor,"bersih"=>$jmlBersih,"wfh"=>$wfh->jumlah);
        }
        $jum_data = $this->pegawai->getDaftar(0,null,null,null,$SQLcari);
        $this_url = site_url('rptuangmakan/pagging/');
    ...
    public function view()
    {
        ...
        $this->generate($jnsLap);
    }

    private function generate($jns=1)
    {
        ...
        $postdatestart = $this->input->post('start'); // $this->input->post('start')."-01";
        $postdatestop = $this->input->post('end'); // date("Y-m-t", strtotime($postdatestart)); //$this->input->post('end');
        ...
        $startdate = date("Y-m-d",strtotime($postdatestart));
        $enddate = date("Y-m-d",strtotime($postdatestop));
        ...
        if($userid!='undefined') {
            ...
            if ($pos === false) {
                $SQLcari .= " and userid in (".$userid.") ";
            } else {
                $aUserID = explode ( ',' , $userid);
                $result = "'" . implode ( "', '", $aUserID ) . "'";
                $SQLcari .= " and userid in (".$result.") ";
            }
        ...
        $query = $this->pegawai->getDaftar(0,null,null,null,$SQLcari,null,"Golru DESC, kelasjabatan DESC");
        ...
        foreach($query->result() as $row)
        {
            ...
            $listdata = $this->pegawai->getlisthistabsensi($row->userid,$startdate,$enddate,$row->deptid)->row();
            $jmlwfh = $this->procmakan->getJmlHariWFH($row->userid,$startdate,$enddate);
			$wfh = $jmlwfh->row();
			$golongan =$row->golru;$jml_pajak=0;$tarif=0;$jum_hadir=0;$jml_kotor=0;$bersih=0;
            if (!empty($listdata))
            {
                $golongan = $listdata->golongan;
                $tarif = $listdata->tarif;
                $jml_pajak = $listdata->jml_pajak;
                $jum_hadir = $listdata->jum_hadir;
				$jml_wfh = $wfh->jumlah;
                $jml_kotor = $listdata->jml_kotor;
                $bersih = $listdata->bersih;
            } else {
				$tarif = $umk["nominal"];
			}
            ...
            /*foreach($listdata->result() as $row2)
            {
                $datapeg[]=array("userid"=>$row->userid,
                    "name"=>$row->name,
                    "golru"=>$row2->golongan,
                    "tarif"=>$row2->tarif,
                    "pajak"=>$row2->jml_pajak,
                    "hadir"=>$row2->jum_hadir,
                    "kotor"=>$row2->jml_kotor,
                    "bersih"=>$row2->bersih);
            }*/

        }
        $data["dptname"]=$org!='undefined'? $deptar[$org]:"";
        $data["result"]=$datapeg;
        $data["tgl"]=$startdate;
		$data["tgled"]=$enddate;
        ...
        $datavw="";
        if($excelid==1) {
            $dataview = $this->load->view("laporan".$nmrpt,$data,true);
            $datavw .= $dataview;
        } else {
            $this->load->view("laporan".$nmrpt,$data);
        }
        ...
        if($excelid==1) {
            ...
            header("Content-Disposition: attachment; filename=laporanuangmakan.xls");
            echo $datavw;
        }
