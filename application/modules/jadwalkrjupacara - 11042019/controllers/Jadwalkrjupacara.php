...
class Jadwalkrjupacara extends MX_Controller {
    ...
    function gethistory()
    {
		...
		$r = date_create($this->input->post('coldate'));
		$t = date_format($r,'Y-m-d');
        $coldate = $t;
		...
		if($coldate!='name' && $coldate!='userid')
        {
			...
			$op = $this->jadwalkrj->getrostertime($userid,$coldate);
            if ($abs->num_rows()>0) {
                $dataarray[0] = array('userid' => $userid, 'name' => $abs->row()->name,
                    'transdate' => date('d-m-Y', strtotime($coldate)),
                    'transtime' => '','absence' => '','kegiatan' => $op,
                    'attendance' => '', 'editby' => $abs->row()->editby);
                $jmlSum ++;
            }
            ...
            if ($att->num_rows()>0) {
                $dataarray[0] = array('userid' => $userid, 'name' => $att->row()->name,
                        'transdate' => date('d-m-Y', strtotime($coldate)),
                        'transtime' => '', 'absence' => '','kegiatan' => $op,
                        'attendance' => $att->row()->atname , 'editby' => $att->row()->editby);
                $jmlSum ++;
            }
            //if ($jmlSum==0)
            {
    ...
    function buatjadwal()
    {
        ...
        if($countuserlist!=0) {
            ...
            foreach ($roster->result() as $rosterdetail) {
                $arrayroster[$rosterdetail->userid][strtotime($rosterdetail->rosterdate)] = array('attendance' => $rosterdetail->attendance);
            }
                ...
                if ($bukatutup) {
                    ...
                    foreach ($userid as $usid) {
                        ...
                        foreach ($result as $row) {
                            ...
                            if (isset($datas)) {
                                createLog("Menghapus checkinout upacara" . $usid . " " . $datas["checktime"], "Sukses");
                            }
