...
class Dashboard_model extends CI_Model
{
                ...
                FROM (
                  ...
                  LEFT JOIN display_data ON ym.y = YEAR(FROM_UNIXTIME(display_data.tgl)) AND 
                    ym.m = MONTH(FROM_UNIXTIME(display_data.tgl)) and ( 
                        attendance = 'AB_1' or attendance  like 'AB_2' or 
                        attendance  like 'AB_4' or attendance  like 'AB_5'
                        or attendance  like 'AB_6')
                    ".$sql2."
                WHERE
                  Y = $tgl
    ...
    function getDisplayData($tgl,$orgId=null)
    {
        ...
        $sql ="
                SELECT 'TEPATWAKTU' AS ket, IFNULL(count(*),0) AS jml 
                FROM view_ontime 
                WHERE date_shift = '$tgl' $addwhere
                UNION ALL
                SELECT 'TERLAMBAT' AS ket, IFNULL(count(*),0) AS jml 
                FROM view_late
                WHERE date_shift = '$tgl' $addwhere
                UNION ALL
                SELECT 'IJIN' AS ket, IFNULL(count(*),0) AS jml 
                FROM view_ijin
                WHERE date_shift = '$tgl' $addwhere
                UNION ALL
                SELECT 'ALPHA' AS ket, IFNULL(count(*),0) AS jml 
                FROM view_alpa
                WHERE date_shift = '$tgl' $addwhere
                UNION ALL
                SELECT 'SAKIT' AS ket, IFNULL(count(*),0) AS jml 
                FROM view_sakit
                WHERE date_shift = '$tgl' $addwhere
                UNION  ALL   
                SELECT 'CUTI' AS ket, IFNULL(count(*),0) AS jml 
                FROM view_cuti
                WHERE date_shift = '$tgl' $addwhere
                UNION  ALL   
                SELECT 'TB' AS ket, IFNULL(count(*),0) AS jml 
                FROM view_tb
                WHERE date_shift = '$tgl' $addwhere
                UNION  ALL   
                SELECT 'DINAS' AS ket, IFNULL(count(*),0) AS jml 
                FROM view_dinas_luar
                WHERE date_shift = '$tgl' $addwhere
                UNION  ALL   
                SELECT 'JUMALL' AS ket, IFNULL(count(*),0) AS jml 
                FROM view_employee
                WHERE jftstatus in (1,2) $addwhere
                UNION  ALL   
                SELECT 'KHS' AS ket, IFNULL(count(*),0) AS jml 
                FROM view_no_jdwl
                WHERE date_shift = '$tgl' $addwhere
                ";
	...
	function getDisplayData_New($tgl,$orgId=null)
    {
