...
<?php
    ...
    if (!is_array($kode_unker))
    {
        if (strpos($kode_unker,","))
        {
            $kode_unker = explode(',',$kode_unker);
        }
    } /*else {
        $kode_unker = array($kode_unker);
    }*/


	//$sql = $this->db->query("SELECT * FROM departments WHERE deptid in $kode_unker ORDER BY deptid ASC");
	$this->db->where_in('deptid', $kode_unker);
...
<?php
	...
	?>
		<div class="tree-folder" style="display: block;">
			<div class="tree-folder-header hd0">
				<i id="fd<?php echo $row->deptid?>" class="icon-plus" onClick="TreeMenu('<?php echo $row->deptid?>',1)"></i>
				<div class="tree-folder-name fn0" onClick="PilihData('<?php echo $row->deptid?>','<?php echo $row->deptname?>')"><?php echo $row->deptname?></div>
				</div>
...
<script>
$(function(){
	var inwidth = $('.hd0:eq(0)').width();
	var blt = (Math.floor(inwidth / 7) - 2);
	var count = $('.fn0');
	for(i=0;i<(count.length);i++){
		var intext = $('.fn0:eq('+i+')').text();
		$('.fn0:eq('+i+')').empty();
		if(intext.length > blt){
			$('.fn0:eq('+i+')').attr('rel','tooltip');
			$('.fn0:eq('+i+')').attr('data-original-title',intext);
		}
		$('.fn0:eq('+i+')').text(intext.substring(0,blt)+((intext.length > blt)?'..':''));
	}

	$('[rel="tooltip"]').tooltip();
});

function TreeMenu(v,lvl)
{
	$('.tree').data('clicked',1);
	var namaClass= document.getElementById('fd'+v).className;
	if(namaClass=='icon-plus'){
		$("#load"+v).css('display','block');
		$.ajax({
			url     : '<?php echo site_url('ajax/child_unkerja_jabodetabek')?>/'+v+'/'+lvl,
			dataType: 'html',
			type    : 'POST',
			success : function(data){
				$("#load"+v).css('display','none');
