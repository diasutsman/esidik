...
<?php
	...
	foreach($sql->result() as $row){
		...
		if($sql2->num_rows() > 0){
			?>
				<div class="tree-folder" style="display: block;">
					<div class="tree-folder-header hda<?php echo $lvl?>">
						<i id="fda<?php echo $row->deptid?>" class="icon-plus" onClick="TreeMenu2('<?php echo $row->deptid?>',<?php echo $lvl+1?>)"></i>
						<div class="tree-folder-name fn<?php echo $lvl?>" onClick="PilihData2('<?php echo $row->deptid?>','<?php echo $row->deptname?>');"><?php echo $row->deptname?></div>
						</div>
		...
		<?php
			...
			<div class="tree-item hda<?php echo $row->deptid?>" style="display: block;" onClick="PilihData2('<?php echo $row->deptid?>','<?php echo $row->deptname?>');">
				<div class="tree-item-name fna<?php echo $row->deptid?>"><?php echo $row->deptname?></div>
			</div>
		<?php
		}
	}
...
<script>
$(function(){
	var inwidth = $('.hda<?php echo $lvl?>:eq(0)').width();
	var blt = (Math.floor(inwidth / 7) - <?php echo $lvl*5?>);

	var count = $('.fna<?php echo $lvl?>');
	for(i=0;i<(count.length);i++){
		var intext = $('.fna<?php echo $lvl?>:eq('+i+')').text();
		$('.fna<?php echo $lvl?>:eq('+i+')').empty();
		if(intext.length > blt){
			$('.fna<?php echo $lvl?>:eq('+i+')').attr('rel','tooltip');
			$('.fna<?php echo $lvl?>:eq('+i+')').attr('data-original-title',intext);
		}
		$('.fna<?php echo $lvl?>:eq('+i+')').text(intext);
	}
