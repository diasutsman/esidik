...
<?php
	...
	$sql = $this->db->query("SELECT * FROM departments WHERE parentid=$param AND wilayah ='1' ORDER BY id ASC");
	foreach($sql->result() as $row){

		$sql2 = $this->db->query("select * from departments WHERE parentid=".$row->deptid." AND wilayah ='1' ORDER BY id ASC");
		if($sql2->num_rows() > 0){
			?>
