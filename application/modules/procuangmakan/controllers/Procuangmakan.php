           ...
           //proses uang makan
            ...
            foreach($queryemp->result() as $row)
            {
                ...
                foreach($querytemp->result() as $que) {
                    ...
                    $this->db->where('idbln', date('n', strtotime($que->date_shift)));
                    $this->db->where('tahun', date('Y', strtotime($que->date_shift)));
                    $query = $this->db->get();
                    if ($query->num_rows()>0) {
                    ...
                    if (!$bukatutup)
                    {
                        ...
                        $this->db->where('tanggal ', date('Y-m-d', strtotime($que->date_shift)));
                        $this->db->delete('data_uang_makan');
                        if ($jmlHdr>0)
                        {
