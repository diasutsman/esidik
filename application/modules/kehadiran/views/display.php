	...
	<div class="ibox float-e-margins">
		...
		<div class="ibox-content">
			<div class="row form-horizontal">
				...
				<div class="form-group">
                    <label class="control-label col-md-2 col-sm-3 col-xs-12" for="statuskehadrian">Status Ketidakhadiran</label>
                    <div class="col-sm-7">
                        <?php
                        $js = 'id="statuskehadrian" class="input-sm form-control chosen-select" name="statuskehadrian" data-placeholder="Pilih Status Kehadiran..."';
                        $selected = array("AT_AT1","AT_AT2");
                        echo form_multiselect('statuskehadrian',$lstKehadiran,$selected,$js);
                        ?>
                    </div>
                </div>
				<div class="form-group">
				...
				<div class="form-group">
					<label class="control-label col-md-2 col-sm-3 col-xs-12" for="start">Tanggal Input</label>
...
<script>
		...
		if(cek > 0){
			bootbox.confirm("Anda yakin menghapus data yang dipilih?", function(result) {
				if(result) {
					...
					$.ajax({
						url     : '<?php echo site_url('kehadiran/hapus');?>',
						dataType: 'json',
						type    : 'POST',
						data    : { 'id' : cek_del},
						success : function(data){
	...
	function saveForm() {
		if (($("#txt1").val() != "")) {
			$.ajax({
				url: '<?php echo site_url('kehadiran/save');?>', dataType: 'json', type: 'POST', data: $("#inputForm").serialize(), success: function (data) {
					bootbox.alert(data.msg);
				}
			});
		} else {
