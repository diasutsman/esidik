...
$url_pag =  site_url("kehadiran/pagging/0");
    ...
    <div class="dataTables_wrapper dt-bootstrap">
        <table class="table table-striped  table-bordered">
            ...
            <tr>
				...
				<th>Nomor (SK/SPT)</th>
                <th width="12%">Tanggal Mulai s/d Akhir</th>
			...
			if ($result>=1)
            {
				foreach($result as $row)
				{
					...
					$namastatus = getValue("atname","attendance","atid='". $row->status."' ");
					$jm=$co;
					...
					<tr>
						...
						<td><?php echo date('d-m-Y', strtotime($row->tanggal_awal))." s/d ".date('d-m-Y', strtotime($row->tanggal_akhir));?></td>
...
<script>
    ...
    function load_url(theurl,orderby,sortingby)
    {
		...
		var par12 = $('#stauskehadiran').val();
        $.ajax({
            method:"post",
            url: theurl,
            data:{"lmt":par4,"cari":par5,"org":par6,"stspeg":par7,"jnspeg":par8,'start':par9,'end':par10,'order':orderby,'sorting':sortingby
			,"stausverifikasi":par11,"stauskehadiran":par12},
	...
	function btnproses(a)
	{
		...
		bootbox.confirm("Yakin Anda Akan Menyetujui Usulan kehadiran Ini ?", function(result) {
			if(result) {
				$.ajax({
					url: '<?php echo site_url('kehadiran/statushadir') ?>',
					dataType: 'json',
					type    : 'POST',
					data:{"org":par6,"stspeg":par7,"jnspeg":par10,"userid":a,"startdate":par8,"enddate":par9},
					success: function(data){
						load_url('<?php echo site_url('kehadiran/pagging/0') ?>', '<?php echo (isset($order)?$order:'id')?>','<?php echo (isset($typeorder)?$typeorder:'sorting')?>');
	...
	function btntolak(a)
	{
		...
		var par7 = $('#stspeg').val();
		var par10 = $('#jnspeg').val();
		bootbox.confirm("Yakin Anda Akan Tidak Menyetujui Usulan Kehadiran Ini ?", function(result) {
			if(result) {
				$.ajax({
					url: '<?php echo site_url('kehadiran/statusditolak') ?>',
					dataType: 'json',
					type    : 'POST',
					data:{"org":par6,"stspeg":par7,"jnspeg":par10,"userid":a,"startdate":par8,"enddate":par9},
					success: function(data){
	...
	function btnkembalikan(a)
	{
		...
		var par7 = $('#stspeg').val();
		var par10 = $('#jnspeg').val();
		bootbox.confirm("Yakin Anda Akan Mengkembalikan Usulan kehadiran Ini ?", function(result) {
			if(result) {
				$.ajax({
					url: '<?php echo site_url('kehadiran/statusdikembalikan') ?>',
					dataType: 'json',
					type    : 'POST',
					data:{"org":par6,"stspeg":par7,"jnspeg":par10,"userid":a,"startdate":par8,"enddate":par9},
					success: function(data){
