...
class Rptatt extends MX_Controller {
    ...
    function index_list()
    {
        ...
        $ogy = $this->pegawai->getDaftar(1,10,$offset,null,$SQLcari);
    ...
    public function pagging($page=0)
    {
        ...
        $ogy = $this->pegawai->getDaftar(1,$limited,$offset,null,$SQLcari);
        $jum_data = $this->pegawai->getDaftar(0,null,null,null,$SQLcari);
    ...
    public function view()
    {
        ...
        $yo = 0;
         if ($userid != 'undefined') {
            ...
            $ogyemp = $this->report_model->getuseremployeedetails($userar,$stspeg,$jnspeg);
            $yo = 2;
            }
        else if ($this->input->post('org') != 'undefined') {
            $ogyemp = $this->report_model->getorgemployeedetails($orgid,$stspeg,$jnspeg);
            $yo = 1;
        }
        if ($selall == 'true') {
            $ogyemp = $this->report_model->getallemployeedetails($stspeg,$jnspeg);
            $yo = 3;
        }
		...
		if ($pilrpt == '1') {
            ...
            for ($x = 0; $x <= $range; $x++) {
                ...
                if ($yo == 1) {
                    $ogytemp = $this->report_model->getattbydateorg($areaid, $tanggal, $orgid,$stspeg,$jnspeg);
                } else if ($yo == 2) {
                    $ogytemp = $this->report_model->getattbydateuser($tanggal, $userar,$stspeg,$jnspeg);
                } else if ($yo == 3) {
                    $ogytemp = $this->report_model->getattbydate($areaid, $tanggal,$stspeg,$jnspeg);
                }

                $dataarray = array();
                foreach ($ogytemp->result() as $que) {
                    ...
                    $wfh = $this->report_model->getdetailrosterWFH($que->userid,$que->date_shift);
					$dataarray[] = array(
                        'userid' => $que->userid,
            ...
            {
                $ogytemp = $this->report_model->getattlog($datestart, $datestop, $queqe['userid']);
				$dataarray = array();
                $totallate = 0;
                $totalearly = 0;
                $totalotbef = 0;
                ...
                foreach ($ogytemp->result() as $que)
                {
                    ...
                    if ($que->attendance=='UPC')
                    {
                        ...
                        $jml = $this->report_model->getstatushadir($que->date_in,$queqe['userid']);
                        if ($jml>0){
                            continue ;
                        }
                    }
                    */
					...
					$dataarray[] = array('day' => $day, 'date' => date('d-m-Y', strtotime($que->date_shift)),
                        'workinghour' => $workinghour, 'activity' => $activity,'activity2' => $upacara,
                        'datein' => $date_in,
                        ...
                        'totalhour' => $totalhour, 'notes' => $notes, 'wfh' => $wfh);
                    $total = $total + $tot;
                }
