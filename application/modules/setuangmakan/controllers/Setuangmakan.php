...
class Setuangmakan extends MX_Controller {

    private $aAkses;

    function Setuangmakan(){
		parent::__construct();
        ...
        $this->aAkses = akses("Setuangmakan", $this->session->userdata('s_access'));
	...
	function index_list()
	{
        ...
        $query = $this->uangmakan->getDaftar(1,10,$offset,null,$SQLcari);
        $jum_data = $this->uangmakan->getDaftar(0,null,null,null,null);
        $this_url = site_url('setuangmakan/pagging/');
    ...
    public function pagging($page=0)
    {
        ...
        if($cr=='cri'){
            ...
            $SQLcari .= " AND ( golongan LIKE '%".str_replace('%20',' ',$cr)."%' 
                        or nominal LIKE '%".str_replace('%20',' ',$cr)."%'  
                        or pajak LIKE '%".str_replace('%20',' ',$cr)."%'  ) ";
        }
        ...
        $query = $this->uangmakan->getDaftar(1,$limited,$offset,null,$SQLcari);
        $jum_data = $this->uangmakan->getDaftar(0,null,null,null,$SQLcari);
        $this_url = site_url("setuangmakan/pagging");
    ...
    {
        echo json_encode($this->uangmakan->getDaftar(0,null,null,$noId,null,null)->row_array());
    }
    ...
    public function save()
    {
        ...
        if ($idx==0)
        {
            $insert = $this->db->insert('ref_uangmakan', $dataIn);
            if (!$insert) {
        ...
        else
        {
            ...
            $update = $this->db->update('ref_uangmakan',$dataIn);
            if(!$update){
