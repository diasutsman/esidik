...
<div class="table-responsive">
    ...
    <table class="table table-striped table-bordered">
        ...
        <tr>
            ...
            <th onClick="load_url('<?php echo $url_pag?>','user_level_name','<?php echo (isset($order)?(($order=='user_level_name')?$sorting:'sorting'):'sorting')?>')" ><div class="<?php echo (isset($order)?(($order=='user_level_name')?$typeorder:'sorting'):'sorting')?>">Keterangan</div></th>
        ...
        foreach($result as $row)
        {
            ...
            <tr id="rowdata-<?php echo $row->user_level_id?>">
                ...
                <td><?php echo $row->lst_menu?></td>
                <td width="80px"><a class="btn btn-xs btn-warning editrow" data-id="<?php echo $row->user_level_id?>"><i class="fa fa-pencil"></i></a>
                    <a title="Privileges" class="btn btn-xs btn-success" href="<?php echo base_url("usraction/indexlist/".$row->user_level_id)?>"><i class="fa fa-anchor"></i></a></td>
            </tr>
            <?php
        ...
        $('.editrow').click(function(){
            ...
            $.getJSON('<?php echo site_url('usrlevel/edit')?>/'+xno,function() {})
                .done(function(response) {
                    ...
                    var mySplitResult;
                    if (response['id_menu'] != null ) {
                        mySplitResult = response['id_menu'].split(',');
                        for (i = 0; i < mySplitResult.length; i++) {
                            $('#menutree').jstree('check_node', '#' + mySplitResult[i]);
                        }
                    }

                    $("#txt6").val(response['id_menu']);
