	   ...
	   // {
            ...
            foreach($shift as $shifting) {
                ...
                if (!$bukatutup) {
                    ...
                    log_message('info', 'Proses tgl. '.$shifting->rosterdate); 
                }
            }
                        ...
                        if(isset($arrai[$userdata][strtotime($tanggal)])) {
                            ...
                            if($attendances != 'AT_') {
                                if(strpos($attendances,'AT_')!==false) {
                                    ...
                                    else if($attendances=='AT_WFH1' ) { $late  = 0; $statusWFH=1; }
                                    else if($attendances=='AT_WFH2' ) { $early = 0; $statusWFH=1; }
                                    else {
                                        $late = 0;
                                        $early = 0;
                                    }

                                    if (isset($akrga[$attendances]))
                                    {
                                        switch ($akrga[$attendances])
                                        {
                                            case 1:
                                                $late = 0;
                                                break;
                            ...
                            $getWFH = $this->process_model->getWFH($userdata, $datein);

                            $attend = '';
                            if($empin!=0 or $empout!=0) {
                                if($workinholiday==0) {
                                    if($absences=='JWFH' OR $absences=='SKWFH' OR $absences=='SKWFH23' OR $absences=='JWFH23'){
                                        if($getWFH == TRUE){
                                            $statusWFH = '1';
                                        }else if($attendances=='AT_WFH1' || $attendances=='AT_WFH2' || $attendances=='AT_WFH3' ){
                                                $statusWFH = '1';
                                            }else{
                                            $statusWFH = '';
                                        }
         ...
         // } else {
   //          $data['msg'] = 'Maaf anda tidak mempunyai akses..';
   //      } 
...

    {
        ...
        //if($this->session->userdata('s_access')==1) 
        {
            ...
            $shift = $this->process_model->getmultishiftuserid($userid, $datestart, $dateend);
            $arrai = array();
            foreach($shift as $shifting) {
                $arrai[$shifting->userid][strtotime($shifting->rosterdate)] = array (
                    'start_in'			=> $shifting->start_in,
                ...
                for($x=0;$x<=$range;$x++) {
                    ...
                    if(isset($arrai[$userdata][strtotime($tanggal)])) {
                        ...
                        if ($allowInsert) {
                            ...
                            $this->db->insert('process_khusus', $savetemporary);
                        } else
                        {
                            $this->db->where('userid', $userdata);
                            $this->db->where('date_shift', strtotime($tanggal));
                            $updatetemporary = array(
                                ...
                                'attendance' => $attend, 'edit_come' => $editin, 'edit_home' => $editout,
                                'notes' => $notes != '' ? $notes : null, 'flaghol' => $flagholiday);
                            $this->db->update('process_khusus', $updatetemporary);
                        }
...

    {
        ...
        //if($this->session->userdata('s_access')==1) 
        {
            ...
            $orgid = $this->pegawai->deptonall($orgidi);
            $range = ($dateend - $datestart) / 86400;
            $shift = $this->process_model->getmultishiftorgid($orgid, $datestart, $dateend);
            $arrai = array();
            foreach($shift as $shifting) {
                $arrai[$shifting->userid][strtotime($shifting->rosterdate)] = array (
                    'start_in'			=> $shifting->start_in,
