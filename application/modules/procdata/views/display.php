			...
			<div class="row form-horizontal">
				...
				<div class="form-group">
					<div class="col-md-10 col-md-offset-2">
						...
						<button type="button" id="btnproses" class="btn btn-sm btn-success"><i class="fa fa-user"></i> Proses Pegawai</button> 
						<button type="button" id="btnsemua" class="btn btn-sm btn-warning"><i class="fa fa-users"></i>  Proses Semua</button>
					</div>
				</div>
			</div>
		</div>
...
<script>
	$(function(){
		...
		$('#btncaridata').click(function(){
			load_url('<?php echo site_url('procdata/pagging/0') ?>', '<?php echo (isset($order)?$order:'id')?>','<?php echo (isset($typeorder)?$typeorder:'sorting')?>');
		});
            ...
            bootbox.confirm("Anda yakin memproses semua data (berdasarkan unit kerja)?", function(result) {
                if(result) {
                    ...
                    $.ajax({
                        dataType: 'json',
                        type    : 'POST',
                        url: '<?php echo site_url('procdata/allpegawai') ?>',
                        data:{"org":par6,"stspeg":par7,"jnspeg":par10,"startdate":par8,"enddate":par9},
                        success: function(data){
                            bootbox.alert(data.msg);
                        },
                        beforeSend: function() {
                            $("#btnsemua").prop('disabled', true);
                            $("#btnsemua").html('<i class="fa fa-key"></i> Processing');
                        },
                        complete: function() {
                            $("#btnsemua").prop('disabled', false);
                            $("#btnsemua").html('<i class="fa fa-repeat"></i> Proses Semua');
                        }
                    });
                }
            });
