        ...
        foreach($rslt as $queq) {
            ...
            if ($isplt==1) {
                ...
                if ( ($jbtnDef  < $jbtnPlt ) ) {
                    ...
                    $unkirplt= isset($mastertunj[$klsjbatan])?$mastertunj[$klsjbatan] :0;
                    $unkir = 0;
                }

                //echo $kriteriaPlt." ".$klsjbatan." ".$unkirplt;
            }
        ...
        foreach($dataallay as $queqe) {
            ...
            foreach($querytemp->result() as $que)
            {
                    ...
                    $que->attendance!='NWDS' && $que->workinholiday!=1 )
                {
                    ...
                    //terlambat
                    if ($check_in == null) {
                        $jumTdkAbsen += (($krglate / 100) * ($tunjangan[date('mY', strtotime($que->date_shift))] * $ttlmsk[date('mY', strtotime($que->date_shift))]));
                        $jumTdkAbsenPlt += (($krglate / 100) * ($tunjanganplt[date('mY', strtotime($que->date_shift))] * $ttlmsk[date('mY', strtotime($que->date_shift))]));
                    } else {
                        $jumlate += (($krglate / 100) * ($tunjangan[date('mY', strtotime($que->date_shift))] * $ttlmsk[date('mY', strtotime($que->date_shift))]));
                        $jumlatePlt += (($krglate / 100) * ($tunjanganplt[date('mY', strtotime($que->date_shift))] * $ttlmsk[date('mY', strtotime($que->date_shift))]));
                    }

                    $totallate += (($krglate /100) * ($tunjangan[date('mY', strtotime($que->date_shift))] * $ttlmsk[date('mY', strtotime($que->date_shift))]));
                ...
                if($krgearly > 0 && $que->attendance!='NWK' && $que->attendance!='NWDS' && $que->workinholiday!=1 ) {
                    ...
                    if ($check_out  == null) {
                        $jumTdkAbsen += (($krgearly / 100) * ($tunjangan[date('mY', strtotime($que->date_shift))] * $ttlmsk[date('mY', strtotime($que->date_shift))]));
                        $jumTdkAbsenPlt += (($krgearly / 100) * ($tunjanganplt[date('mY', strtotime($que->date_shift))] * $ttlmsk[date('mY', strtotime($que->date_shift))]));
                    } else {
                        $jumearly += (($krgearly / 100) * ($tunjangan[date('mY', strtotime($que->date_shift))] * $ttlmsk[date('mY', strtotime($que->date_shift))]));
                ...
                if($krgalpa != 0 && $que->workinholiday!=1) {
                    ...
                    //alpha
                    $jumlalpa += (($krgalpa /100) * ($tunjangan[date('mY', strtotime($que->date_shift))] * $ttlmsk[date('mY', strtotime($que->date_shift))]));
                ...
                if($krgstatus != 0  && $queqe['payable']!=0) {
                    ...
                    $jumijin += (($krgstatus /100) * ($tunjangan[date('mY', strtotime($que->date_shift))] * $ttlmsk[date('mY', strtotime($que->date_shift))]));
                ...
                */
                if (isset($que->date_shift2) && !isset($que->date_in2) 
                    && !isset($que->check_in2) && $queqe['payable'] !=0 && $tubel==0)
                {
                    if($que->attendance!='AB_12') //jika bukan status ketidak hadiran
                    {
                        $nkurang = refUpacara();
                        $totalpersenkurang = $totalpersenkurang + $nkurang;
            ...
            $dataallaye[] = array(
                ...
                "totaltunjplt" => $totaltunjplt,
                'jmlsikerja' =>       $nSiKerja,
                'potupacara' 			=> $jumUpacara> 0 ? $jumUpacaraPlt : 0,
                ...
                */
                    ...
                    && !isset($que->check_in2) && $queqe['payable'] !=0 && $tubel==0)
                {
                    if(substr($que->attendance,0,2)!='AB') //jika bukan status ketidak hadiran
                    {
                        ...
                        $tunj = $tunj + (($nkurang /100) * ($tunjangan[date('mY', strtotime($que->date_shift))] * $ttlmsk[date('mY', strtotime($que->date_shift))]));
                        $tunjplt = $tunjplt + (($nkurang /100) * ($tunjanganplt[date('mY', strtotime($que->date_shift))] * $ttlmsk[date('mY', strtotime($que->date_shift))]));
                        $tunjlama = $tunjlama + (($nkurang /100) * ($tunjanganlama[date('mY', strtotime($que->date_shift))] * $ttlmsk[date('mY', strtotime($que->date_shift))]));
                        $tunjpltlama = $tunjplt + (($nkurang /100) * ($tunjanganpltlama[date('mY', strtotime($que->date_shift))] * $ttlmsk[date('mY', strtotime($que->date_shift))]));
                    }
                }
