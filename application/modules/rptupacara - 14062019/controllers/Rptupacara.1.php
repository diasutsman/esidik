        ...
        if ($pilrpt == '1') 
        {
            ...
            for ($x = 0; $x <= $range; $x++) {
                ...
                foreach ($querytemp->result() as $que) 
                {
                    ...
                    $upacara='';
                    if (isset($que->date_in)){
                        $upacara=$que->attendance=='UPC'?'Ya':'Tidak';
                    }
                    $cekinupcara="";
                    $qa = $this->report_model->gettranslog2($que->date_shift,$que->date_shift,$que->userid,"Upacara");
                    $za = $qa->first_row();
                    if ($za != '' or $za != null) {
                        $cekinupcara= date('d-m-Y H:i:s', strtotime($za->checktime));  
                    }
            ...
            foreach ($dataallay as $queqe) 
            {
                ...
                foreach ($querytemp->result() as $que)
                {
                    ...
                    $cekinupcara="";
                    $qa = $this->report_model->gettranslog2($que->date_shift,$que->date_shift,$queqe['userid'],"Upacara");
                    $za = $qa->first_row();
                    if ($za != '' or $za != null) {
                        $cekinupcara= date('d-m-Y H:i:s', strtotime($za->checktime));  
                    }
