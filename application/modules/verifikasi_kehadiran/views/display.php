			...
			<div class="row form-horizontal">
				...
				<div class="form-group">
                    ...
                    <div class="col-sm-7">
                        ...
                        $selected = array("30","29");
                        echo form_multiselect('stausketidakhadiran',$lstStsDar,$selected,$js);
                        ?>
                    </div>
                </div>
				<div class="form-group">
...
<script>
		...
		if(cek > 0){
			bootbox.confirm("Anda yakin menghapus data yang dipilih?", function(result) {
				if(result) {
					...
					$.ajax({
						url     : '<?php echo site_url('verifikasi_kehadiran/hapus');?>',
						dataType: 'json',
						type    : 'POST',
						data    : { 'id' : cek_del},
						success : function(data){
	...
	function saveForm() {
		if (($("#txt1").val() != "")) {
			$.ajax({
				url: '<?php echo site_url('verifikasi_kehadiran/save');?>', dataType: 'json', type: 'POST', data: $("#inputForm").serialize(), success: function (data) {
					bootbox.alert(data.msg);
				}
			});
		} else {
