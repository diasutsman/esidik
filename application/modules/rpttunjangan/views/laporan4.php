...
<center>
    ...
    <h1>
            ...
            <br><div >PERIODE : <?php echo $dateinfo;?></div>
        </h1>
</center><br>
    <center></center><br>
<?php //}?>
    <table class="head">
        <tr>
            ...
            <td style="border:0px" class="text">: <?php echo $empinfo[0]['deptName']; ?></td>
    ...
    <table width="100%" border="1" cellspacing="0" cellpadding="0">
        ...
        <tr>
            ...
            <th colspan="6" align="center"  class="text">POTONGAN</th>
        ...
        foreach($empinfo as $row) {
            ...
            //if ($row['isViewIt'])
            //{
                //echo $row['ishidden'].' '.$row['userid'].' '.$tunjangan.'<br>';
                $telat = ceil(round($row['potterlambat'], 2));
                $psw = ceil(round($row['potpsw'], 2));
                $ijin = ceil(round($row['potijin'], 2));
                $alpa = (ceil(round($row['potalpa'], 2)) > ceil(round($row['tunjangan'], 2))) ? ceil(round($row['tunjangan'], 2)) : ceil(round($row['potalpa'], 2));
                $cuti = ceil(round($row['potcuti'], 2));
                $tb = ceil(round($row['pottb'], 2));
                $jam = ceil(round($row['potjam'], 2));
                $tdkabsen = ceil(round($row['pottdkabsen'], 2));
                $tdkabsenplt = ceil(round($row['pottdkabsenplt'], 2));
                $alpaplt = ceil(round($row['potalpaplt'], 2));
                $telatplt = ceil(round($row['potterlambatplt'], 2));
                $pswplt = ceil(round($row['potpswplt'], 2));
                $ijinplt = ceil(round($row['potijinplt'], 2));
                $cutiplt = ceil(round($row['potcutiplt'], 2));
                $tbplt = ceil(round($row['pottbplt'], 2));
                $potupacara = ceil(round($row['potupacara'], 2));
                $potupacaraplt = ceil(round($row['potupacaraplt'], 2));

                //$pengurang = $totaltunjangan + $totaltunjanganplt;
                // var_dump($row['kriteriaPlt']);
                // var_dump($row['kelasjabatan']);
                // var_dump($row['dipisah']);
                // var_dump($row['plt_kelasjabatan']);
                $pengurang = $telat + $alpa + $psw + $tb +$tdkabsen + $potupacara;
                ...
                if ($row['isplt'] != 1) {
                    ...
                    if (($row['kriteriaPlt'] == 1) || ($row['kriteriaPlt'] == 2)) {
                        ...
                        if ($row['kriteriaPlt'] == 2){
                            $kelas = $row['plt_kelasjabatan'] == 0 ? '' : ($row['plt_kelasjabatan']>$row['kelasjabatan']?$row['plt_kelasjabatan'] : $row['kelasjabatan']);

                            if ($row['dipisah'] == 2 && $row['plt_kelasjabatan']>=$row['kelasjabatan'] ) {
                                $tunjangan = $tunjanganplt;
                                $totaltunjangan = $totaltunjanganplt;

                                $tdkabsen=$tdkabsenplt;
                                $alpa = $alpaplt;
                                $telat = $telatplt;
                                $psw = $pswplt;
                                $ijin = $ijinplt;
                                $cuti = $cutiplt;
                                $tb = $tbplt;
                                $potupacara = $potupacaraplt;
                                $pengurang = $telat + $alpa + $psw + $tb + $tdkabsen + $potupacara;
                                $total = $tunjanganplt- $pengurang;
                            }

                            if ($row['dipisah'] == 1){
                                $kelas = $row['kelasjabatan'];
                                $pengurang = $telat + $alpa + $psw + $tb + $tdkabsen + $potupacara;
                                $total = $tunjangan- $pengurang;
                            }
                    ...
                    if (($row['kriteriaPlt'] == 3)) {
                        ...
                        if ($row['dipisah'] == 2) {
                            //$tunjangan = 0;
                           // $totaltunjangan = 0;
                            //echo $row['userid'].' '.$tunjangan.'5<br>';
                        }
                    }
                ...
                //echo $row['userid'].' '.$tunjangan.' '.$row['isplt'].' '.$row['dipisah'].' '.$row['kriteriaPlt'].'<br> ';
                ...
                $thp = $total + $nsikerja;
                $nPPn = round((($total + $nsikerja) * $nPot),2);
                $totalsikerja += $nsikerja;
                $totalThp += $thp;
                $totalPpn += $nPPn;
                //var_dump($tunjangan);die;
                ?>
                <tr>
                ...
                <td align="right" class="num"><?php /*echo $excelid==1?$total:number_format($total,2,'.',','); */ ?></td>-->
                    <td align="right"
                        class="num"><?php echo $excelid == 1 ? $telat : number_format($telat, 2, '.', ',');; ?></td>
                    <td align="right"
                        class="num"><?php echo $excelid == 1 ? $psw : number_format($psw, 2, '.', ',');; ?></td>
                    ...
                    <td align="right"
                        class="num"><?php echo $excelid == 1 ? $alpa : number_format($alpa, 2, '.', ',');; ?></td>
                    <td align="right"
                        class="num"><?php echo $excelid == 1 ? $tb : number_format($tb, 2, '.', ',');; ?></td>
                    <td align="right"
                        class="num"><?php echo $excelid == 1 ? $tdkabsen : number_format($tdkabsen, 2, '.', ','); ?></td>
                    <td align="right"
                        class="num"><?php echo $excelid == 1 ? $potupacara : number_format($potupacara, 2, '.', ','); ?></td>
                    <td align="right"
                    ...
                    <td align="right"
                        class="num"><?php echo $excelid == 1 ? $nsikerja : number_format($nsikerja, 2, '.', ','); ?></td>
                    <td align="right"
                        class="num"><?php echo $excelid == 1 ? $thp : number_format($thp, 2, '.', ','); ?></td>
                    <td align="right"
                        class="num"><?php echo $excelid == 1 ? $nPPn : number_format($nPPn, 2, '.', ','); ?></td>
    ...
    <div style="margin-top: 75px">
        <table border="0" style="border: none; width: 100%">
            ...
            <tr>
                ...
                <td width="6%" class="ttd"></td>
                <td width="6%" class="ttd"></td>
                <td colspan="7" width="35%" class="text ttd" style="padding-bottom: 100px;"><b><?php echo  $ttd_jabatan; ?></b></td>
            </tr>
            <tr></tr>
            <tr></tr>
            <tr></tr>
