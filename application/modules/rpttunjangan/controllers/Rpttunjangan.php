...
class Rpttunjangan extends MX_Controller {
    ...
    function view1()
    {
        ...
        foreach($holiday->result() as $hol) {
            ...
            if($selisih==0) {
                $flaghol[$hol->deptid][$hol->startdate] = $hol->info;
            } else {
                $jarak = $selisih / 86400;
                for($k=0;$k<=$jarak;$k++) {
        ...
        foreach($rslt as $queq) {
            ...
            $DefHist = $this->report_model->getkelasdeftukinhist($queq->userid,$postdatestart);
            ...
            if (isset($PltHist['tmt']) && isset($PltHist['deptid']) && isset($PltHist['eselon']))
            {
                if ($PltHist['tmt'] !='0000-00-00') {
					...
					//cek PLT OFF
                    $PltOffHist = $this->report_model->getkelaspltOfftukinhist($queq->userid,$postdatestart);
					
                    if (isset($PltOffHist['tmt']) && isset($PltOffHist['deptid']) && isset($PltOffHist['eselon']))
                    {
                        if ($PltOffHist['tmt'] !='0000-00-00') {
                            $time = strtotime($PltOffHist['tmt']);
                            /*if ($datestart >= strtotime(date("Y-m-01",$time)))
                            {*/
                                $PltHist=array('tmt'=>null);
                                $isplt = 0;
                            //}
                        }  
                    }
            ...
            $dftKelasJabatan = isset($DefHist['kelas']) ? $DefHist['kelas']:$queq->kelas;
			...
			if ($isplt==1) {
                ...
                if ($jbtnDef  == $jbtnPlt) {
                    ...
                    if ($pltKelasJabatan > $queq->kelasjabatan) {
                        ...
                        $unkirdef = isset($mastertunj[$dftKelasJabatan]) ? $mastertunj[$dftKelasJabatan] : 0;
                        $unkirdef = ($unkirdef * refTHP());
                        $unkir =  $unkirdef*0.2;
            ...
            $dataallay[] = array(
                ...
                'isViewIt'=>$isViewIt,
                'orgf'=>"",
                'status_skjabat' => $queq->status_skjabat,
                'tmt_skjabat' => $queq->tmt_skjabat,
            );
            //var_dump($dataallay);
		...
		foreach($dataallay as $queqe) {
            ...
            foreach($querytempo->result() as $que) 
            {
				...
				if($tunjang!=0) {
                    ...
                    //var_dump($queqe['kriteriaPlt']);
                    //var_dump($queqe['kelasjabatan']);
                    ...
                    //echo $queqe['jftstatus']." ".$queqe['tunjangan'].'<br>';
                    //JFT aktif mulai 1 Juni 2019
                    $bypasstime=strtotime('2019-06-01');
                    if ($curenttime>=$bypasstime){

                        $jftHist = $this->report_model->getjfthistory($queqe['userid'], $que->date_shift);
                        if ($jftHist != null)
                        {
                            if ( strtotime($jftHist) >= strtotime($que->date_shift))
                            {
                                $queqe['tunjangan'] = $queqe['tunjangan'] * refTHPJFT();
                            }
                        } else {
                            if ($queqe['jenisjabatan']==2 && $queqe['status_skjabat']==0 
                                && strtotime($queqe['tmt_skjabat'] >= strtotime(date('Y-m-d')) ))
                            {
                                $queqe['tunjangan'] = $queqe['tunjangan'] * refTHPJFT();
                            }
                        }
            ...
            foreach($querytemp->result() as $que) {
				...
				echo isset($tunbul[date('mY', strtotime($que->date_shift))])."(".$que->date_shift.")[".date('j', strtotime($que->date_shift))."]|";
				if(isset($tunbul[date('mY', strtotime($que->date_shift))])) {
                    //var_dump(isset($tunbul[date('mY', strtotime($que->date_shift))][date('j', strtotime($que->date_shift))]));
                    //var_dump([date('j', strtotime($que->date_shift))]);
					echo "masuk sini pre!";
                    if(isset($tunbul[date('mY', strtotime($que->date_shift))][date('j', strtotime($que->date_shift))])) {
						...
						//var_dump($ttlmsk[date('mY', strtotime($que->date_shift))]);
						...
						//echo $tunbul[date('mY', strtotime($que->date_shift))][date('j', strtotime($que->date_shift))];
                        //echo $tunjangan[date('mY', strtotime($que->date_shift))];
                    }
                } else {
				...
				//echo $que->date_shift;
            //echo $tunjangan[date('mY', strtotime($que->date_shift))]."<br>";
				...
				// echo isset($tunprobul[date('mY', strtotime($que->date_shift))][date('j', strtotime($que->date_shift))])."/";
                ...
                if($que->late!=0) {
                    ...
                    } else
                    {
                        if($late < 3660) { // < 61 menit
                            $krglate = 0.5;
                            //if ($que->ot_after>=$late) $krglate = 0;
                ...
                //var_dump($que->attendance);
                if(isset($atar[$que->attendance])) {
                    if($que->attendance=='AT_SK01' || $que->attendance=='AT_AT1' || $que->attendance=='AT_WFH1')
                        $krglate = 0;
                    else if($que->attendance=='AT_SK03' || $que->attendance=='AT_AT2' || $que->attendance=='AT_WFH2')
                        $krgearly = 0;
                    else {
                        $krglate = 0;
                        $krgearly = 0;
                ...
                */ 
                ...
                $arrAtt=array("AT_AT1","AT_SK01","AT_AT5","AT_AT6","AT_WFH1"); //tidak dipotong
                $kdAbsPtg="AB_12";//dipotong

                //ada jadwal,dibayarkan,tidak dalam tubel,tidak kondisi alpa
                if ($que->date_shift2!="" && $queqe['payable'] !=0  && $tubel==0 && $que->attendance != 'ALP')
                {			
                    //holiday cek
                    $flagholiday = 0;
                    if( isset($flaghol[$queqe['deptid']][date('Y-m-d', strtotime($que->date_shift2))]) ||
                        //isset($flaghol[$queqe['plt_deptid']][date('Y-m-d', strtotime($que->date_shift2))]) || 
                        isset($flaghol['1'][date('Y-m-d', strtotime($que->date_shift2))])) $flagholiday = 1;

                    //print_r($que->date_shift2);
                    //$qa = $this->report_model->gettranslog2($que->date_shift2,$que->date_shift2,$que->userid,"Upacara");
                    $qa = $this->report_model->getattlogupacara(strtotime($que->date_shift2),strtotime($que->date_shift2),$que->userid);
                    $za = $qa->first_row();

                    //if ($que->date_in2!="" && $que->check_in2 !="" ) { //ada tap ke mesin
                    if ($za != '' or $za != null) //jika ada log hasil proses upacara
                    {
                        
                        $jwdlawal = strtotime($za->date_shift.' '.$za->shift_in);
                        $jwdlakhir = strtotime($za->date_shift.' '.$za->shift_out);

                        $cekinupcara= strtotime($za->checktime);
                        if ( ($cekinupcara >= $jwdlawal) && ($cekinupcara <= $jwdlakhir) ) //jika tap in dalam range jadwal upacara
                        {
                            ...
                            if (in_array($za->attendance,$arrAtt))
                            {
                                $ispotong=0; 
                            } else {
                                 if (substr($za->attendance,0,2)=="AB"){
                                    if ($kdAbsPtg==$za->attendance){
                                        $ispotong=1; 
                                    } 
                                    else {
                                        $ispotong=0; 
                                ...
                                if ($za->attendance=="CPC")
                                {
                                    $ispotong=1; 
                                }
                            }

                            $ispotong = ($flagholiday==1 ) ? 0 : $ispotong;
                            if ($ispotong==1){
                                $csts = 'Tidak melakukan upacara ';
                                if ($za->attendance=="CPC")
                                {
                                    $csts = 'Pembatalan Absensi Upacara ';
                                }
                                $nkurang = refUpacara();
                                $dataarray[] = array(
                                    'day'			=> $day,
                        ...
                        {
                            //jika diluar range waktu jadwal upacara
                            $jwdlawal = date('Y-m-d H:i:s',$jwdlawal);
                            $jwdlakhir = date('Y-m-d H:i:s',$jwdlakhir);
                            $cekinupcara = date('Y-m-d H:i:s',$cekinupcara);
                            $ispotong=1;
            ...
            $data = array(
                ...
                "s_user_level_id" => $this->session->userdata('s_user_level_id'),
    ...
    function view2()
    {
        ...
        foreach($dataallay as $queqe) {
            ...
            foreach($querytemp->result() as $que) 
            {
                ...
                if(isset($kedu[date('mY', strtotime($que->date_shift))][date('j', strtotime($que->date_shift))])) {
					...
					if($kedu[date('mY', strtotime($que->date_shift))][date('j', strtotime($que->date_shift))] == 4 OR $queqe['kedudukan'] == 4) {
                        if ($datestart > strtotime(date("Y-m-01",strtotime($queqe['tmtkedudukan'])))) {
                            $tubel = 1;
                            $tgltubel = strtotime($queqe['tmtkedudukan']);
                        }
			...
			if($queqe['jftstatus'] == 1 AND $queqe['empTitle'] != ''){
				$to = $tottun2==0?0:$tunj1;
			}else{
				$to = $tottun2==0?0:$tunj;
			}
			
			if($userid != 'undefined'){
				$namaunitkerja = $queqe['deptName'];
			}else{
        ...
        $data = array(
            ...
            "cominfo" =>    $company,
            "empinfo" =>    $dataallaye,
            "data" =>       isset($deptar[$departemen])?$deptar[$departemen]:"",
            "excelid"=>     $excelid,
            "pdfid"=>     $ispdf,
            "showpdf"=>$showpdf,
            "s_user_level_id" => $this->session->userdata('s_user_level_id'),
    ...
    function view3()
    {
        ...
        foreach($dataallay as $queqe) {
			...
			if($tubel==1) {
                ...
                $tottun2 = ($totaltubel/$totalmasuk);///$totalmasuk);
                $tunj = 0;
                $totplt= $tunjtubelplt;
            ...
            $dataallaye[] = array(
                ...
                'tunjangan' 			=> $tottun2,
                'tunjanganplt' =>       $totplt,
                'totaltunjangan'		=> $tottun2==0?0:$tunj,
                'totaltunjanganplt'		=> $totplt==0?0:$tunjplt,
                "totaltunj" =>$totaltunj,
                "totaltunjplt" => $totaltunjplt,
                'jmlsikerja' =>       $nSiKerja,
            ...
            foreach($querytemp->result() as $que)
            {
                ...
                if(isset($atar[$que->attendance])) {
                    if($que->attendance=='AT_SK01' || $que->attendance=='AT_AT1')
                        ...
                        $krgearly = 0;
                    else if($que->attendance=='AT_WFH1' || $que->attendance=='AT_WFH2')
                        $krgearly = 0;
                    else {
                        $krglate = 0;
                        $krgearly = 0;
        ...
        foreach($dataallay as $queqe) {
            ...
            foreach($querytemp->result() as $que) {
                ...
                */ 
                ...
                {		
                    $flagholiday = 0;
                    //if(isset($flaghol['1'][date('Y-m-d', strtotime($que->date_shift2))])) $flagholiday = 1;
                    if( isset($flaghol[$queqe['deptid']][date('Y-m-d', strtotime($que->date_shift2))]) ||
                        //isset($flaghol[$queqe['plt_deptid']][date('Y-m-d', strtotime($que->date_shift2))]) || 
                        isset($flaghol['1'][date('Y-m-d', strtotime($que->date_shift2))])) $flagholiday = 1;
                    ...
                    if($za != '' or $za != null) //cek jadwal
                    {
                        ...
                        if ($cekinupcara < $jwdlawal  || $cekinupcara > $jwdlakhir)
                        {
                            ...
                            $ispotong = ($flagholiday==1 && $ispotong==1) ? 0 : $ispotong;
