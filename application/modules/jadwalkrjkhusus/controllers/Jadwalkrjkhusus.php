...
class Jadwalkrjkhusus extends MX_Controller {

    private $aAkses;

	function Jadwalkrjkhusus(){
		parent::__construct();
		...
		$this->load->model('jadwalkrjkhusus_model','jadwalkrjkhusus');
        ...
        $this->aAkses = akses("Jadwalkrjkhusus", $this->session->userdata('s_access'));
    ...
    function index_list()
    {
        ...
        $data2 = $this->mypagination->getPagination(0,10,site_url('jadwalkrjkhusus/pagging/'),$uri_segment);
    ...
    public function pagging($page=0)
    {
        ...
        if ( $this->session->userdata("result"))
        {
            ...
            $data2 = $this->mypagination->getPagination($this->session->userdata("jum_data"), $limited, site_url("jadwalkrjkhusus/pagging"), 3);
            $data['paging'] = $data2['link'];
            $data['offset'] = $offset;
        ...
        {
            $data2 = $this->mypagination->getPagination(0,10,site_url('jadwalkrjkhusus/pagging'),3);
            $data['paging'] = $data2['link'];
            $data['offset'] = 0;
            $data['jum_data'] = 0;
            $data['arrcolor'] = array();
    ...
    function gethistory()
    {
        ...
        {
            $jmlSum=0;
            $abs = $this->jadwalkrjkhusus->getabsen($userid, $coldate);
            //print_r($this->db->last_query());
            //print_r($abs->row_array());

            if ($abs->num_rows()>0) {
                $arrabse1=array( $abs->row()->abname!="" ?"1. ".$abs->row()->abname:"",
                    $abs->row()->abname_1!="" ?"2. ".$abs->row()->abname_1:"",
                    $abs->row()->abname_2!="" ?"3. ".$abs->row()->abname_2:"");
                $arredt1=array($abs->row()->editby,$abs->row()->editby_1,$abs->row()->editby_2);
                $dataarray[0] = array('userid' => $userid, 'name' => $abs->row()->name,
                    'transdate' => date('d-m-Y', strtotime($coldate)),
                    'transtime' => '',
                    'absence' => implode(" ",$arrabse1),
                    'attendance' => '',
                    'editby' =>implode(" ",$arredt1) );
                $jmlSum ++;
            }

            $att = $this->jadwalkrjkhusus->getattend($userid, $coldate);
            //print_r($this->db->last_query());
            if ($att->num_rows()>0) {
                $arrabse2=array( $att->row()->atname!="" ?"1. ".$att->row()->atname:"",
                    $att->row()->atname_1!="" ?"2. ".$att->row()->atname_1:"",
                    $att->row()->atname_2!="" ?"3. ".$att->row()->atname_2:"");
                $arredt2=array($att->row()->editby,$att->row()->editby_1,$att->row()->editby_2);

                if (isset($dataarray[0]["absence"]) && $dataarray[0]["absence"]!="")
                {
                    $data3= $dataarray[0]["absence"];
                }
                else {
                    $data3="";
                }
                $dataarray[0] = array('userid' => $userid, 'name' => $att->row()->name,
                    'transdate' => date('d-m-Y', strtotime($coldate)),
                    'transtime' => '',
                    'absence' => $data3,
                    'attendance' => implode(" ",$arrabse2),
                    'editby' => implode(" ",$arredt2));
                $jmlSum ++;
            }
            if ($jmlSum==0) {
                $translog = $this->jadwalkrjkhusus->gettranslog($userid, $coldate);
    ...
    function buatjadwal()
    {
        ...
        if($countuserlist!=0) {
            ...
            $roster = $this->jadwalkrjkhusus->getroster($orgid, $datestart, $dateend);
            //print_r($this->db->last_query());
            $arrayroster = array();
            foreach ($roster->result() as $rosterdetail) {
                $arrayroster[$rosterdetail->userid][strtotime($rosterdetail->rosterdate)] =
                    array('absence' => $rosterdetail->absence, 'attendance' => $rosterdetail->attendance,
                        'absence_1' => $rosterdetail->absence_1, 'attendance_1' => $rosterdetail->attendance_1,
                        'absence_2' => $rosterdetail->absence_2, 'attendance_2' => $rosterdetail->attendance_2);
            }

            //print_r($roster);

            $holiday = $this->jadwalkrjkhusus->holiday($orgid);
            ...
            foreach ($userlist->result() as $datauser) {
                ...
                for ($j = 0; $j <= $range; $j++) {
                    if (isset($arrayroster[$datauser->userid][$datestart + ($j * 86400)])) {
                        ...
                        $shiftname2 = $arrayroster[$datauser->userid][$datestart + ($j * 86400)]['absence_1'];
                        $absattstat2 = $arrayroster[$datauser->userid][$datestart + ($j * 86400)]['attendance_1'];

                        $shiftname3 = $arrayroster[$datauser->userid][$datestart + ($j * 86400)]['absence_2'];
                        $absattstat3 = $arrayroster[$datauser->userid][$datestart + ($j * 86400)]['attendance_2'];
                        ...
                        if ($shiftname) {
                            ...
                            if (in_array(date('Y-m-d', $datestart + ($j * 86400)), $holarray))
                                $sts = '';
                        }
                        array_push($stas, $sts);
                        if ($shiftname2) {
                            if (isset($absattstat2)) {
                                if($absattstat2=='NWDS')
                                    $sts =$shiftname2.'#'.$absattstat2;
                                else {
                                    if(array_key_exists($absattstat2, $atar))
                                        $sts =$shiftname2.'#AT';
                                    else if(array_key_exists($absattstat2, $bbar))
                                        $sts =$shiftname2.'#AB';
                                    else
                                        $sts =$shiftname2;
                                }
                        ...
                        array_push($stas, $sts);
                        if ($shiftname3) {
                            if (isset($absattstat3)) {
                                if($absattstat3=='NWDS')
                                    $sts =$shiftname3.'#'.$absattstat3;
                                else {
                                    if(array_key_exists($absattstat3, $atar))
                                        $sts=$shiftname3.'#AT';
                                    else if(array_key_exists($absattstat3, $bbar))
                                        $sts=$shiftname3.'#AB';
                                    else
                                        $sts=$shiftname3;
                                }
                        ...
                        array_push($stas, $sts);
                        $sts = implode('|',$stas);
                        $data_arr[($datestart + ($j * 86400))] =$sts;
        ...
        $data2 = $this->mypagination->getPagination($jum_data,10,site_url("jadwalkrjkhusus/pagging"),3);
        $data['paging'] = $data2['link'];
        $data['offset'] = 0;
    ...
    function form1($tgl1=null,$tgl2=null,$shifid=1)
    {
    ...
    function form2($tgl1=null,$tgl2=null,$shifid=1)
    {
        $data["tgl1"] = ($tgl1==null? date("d-m-Y") : date("d-m-Y",strtotime($tgl1)));
        $data["tgl2"] = ($tgl2==null? date("d-m-Y") : date("d-m-Y",strtotime($tgl2)));
    ...
    function form3($tgl1=null,$tgl2=null,$shifid=1)
    {
        $data["tgl1"] = ($tgl1==null? date("d-m-Y") : date("d-m-Y",strtotime($tgl1)));
        $data["tgl2"] = ($tgl2==null? date("d-m-Y") : date("d-m-Y",strtotime($tgl2)));
        $data["shifid"]= $shifid;
    ...
    public function statushadir()
    {
        ...
        $noshift = $this->input->post('shiftid');
        $startattilog = '';
        $endattilog = '';
        ...
        for($j=0;$j<=$range;$j++) {
            ...
            if ($query->num_rows()>0) {
                ...
                if ($bukatutup) {
                    ...
                    $rsl = $this->db->get("rosterdetailsatt_khusus");
                    ...
                    if ($rsl->num_rows() > 0) {
                        ...
                        foreach ($result as $row) {
                            ...
                            $query = $this->db->get("rosterdetailsatt_khusus");
                            $datas = $query->row_array();
                            log_history("edit", "rosterdetailsatt_khusus", $datas);
                        ...
                        $dataupdater = array(
                            'attendance'.$nm => $attcode,
                            'notes'.$nm => $notes,
                            'nosk'.$nm => $nosk,
                            'editby'.$nm => $this->session->userdata('s_username')
                        );
                        $this->db->where('userid', $userid);
                        $this->db->where('rosterdate', $atdate);
                        $this->db->update('rosterdetailsatt_khusus', $dataupdater);
                        ...
                        $this->db->insert('rosterdetailsatt_khusus', $dataupdate);
                        $jmlProc++;
            ...
            if ($query->num_rows()>0) {
                ...
                if ($bukatutup) {
                    ...
                    if ($rsl->num_rows() > 0) {
                        ...
                        $dataupdater = array(
                            'attendance'.$nm => $abscode, 'notes'.$nm => $notes,
                            'editby'.$nm => $this->session->userdata('s_username'),
                            'nosk'.$nm => $nosk);
                        $this->db->where('userid', $userid);
                        $this->db->where('rosterdate', $atdate);
        ...
        for($j=0;$j<=$range;$j++) {
            ...
            if ($query->num_rows()>0) {
                ...
                if ($bukatutup) {
                    ...
                    $result = $this->db->get('rosterdetailsatt_khusus')->result();
                    foreach ($result as $row) {
                        $this->db->where('id', $row->id);
                        $query = $this->db->get("rosterdetailsatt_khusus");
                        $datas = $query->row_array();
                        log_history("delete", "rosterdetailsatt_khusus", $datas);
                    ...
                    $this->db->group_end();*/
                    $this->db->where('rosterdate', $abdate);
                    $this->db->delete('rosterdetailsatt_khusus');
                    ...
                    foreach ($userid as $usid) {
                        ...
                        foreach ($result as $row) {
                            ...
                            if (isset($datas)) {
                                createLog("Menghapus checkinout " . $usid . " " . $datas["checktime"], "Sukses");
                            }
                        }
                        ...
                        createLog("Menghapus checkinout " . $usid . " " . $tgal, "Sukses");

                        //createLog("Menghapus checkinout ".$usid." ". date('Y-m-d', $tgal),"Sukses");

                        $arr[] = ["id" => $usid . "-" . date('Y-m-d', $tgal)];
                    }
                }
            }
