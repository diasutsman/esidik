...
class Jadwalkrj extends MX_Controller {
	...
	function Jadwalkrj(){
        ...
        $this->aAkses = akses("Jadwalkrj", $this->session->userdata('s_access'));
        ...
        if($coldate!='name' && $coldate!='userid')
        {
            ...
            if ($abs->num_rows()>0) {
                $dataarray[0] = array('userid' => $userid, 'name' => $abs->row()->name,
                    ...
                    'transtime' => '',
                    'absence' => $abs->row()->abname , 'attendance' => '', 'editby' => $abs->row()->editby,
                    'abid' => $abs->row()->abid);
                $jmlSum ++;
            }
            ...
            if ($att->num_rows()>0) {
                $dataarray[0] = array('userid' => $userid, 'name' => $att->row()->name,
                        'transdate' => date('d-m-Y', strtotime($coldate)),
                        'transtime' => '', 'absence' => '',
                        'attendance' => $att->row()->atname ,
                        'atid' => $att->row()->atid , 'editby' => $att->row()->editby);
                $jmlSum ++;
            }
            //var_dump($coldate);die;
            //if ($jmlSum==0)
            {
        ...
        for($j=0;$j<=$range;$j++) {
            ...
            if ($query->num_rows()>0) {
                ...
                if ($bukatutup) {
                    ...
                    $msg = 'Periode Bulan '.date('n', $tgal).' Tahun '.date('Y', $tgal).' Sudah Lewat 7 Hari dan Ditutup!!';
                }
            }
