        ...
        if ($pilrpt == '1') 
        {
            ...
            for ($x = 0; $x <= $range; $x++) {
                ...
                foreach ($querytemp->result() as $que) 
                {
                    ...
                    $op = $this->report_model->gettranslog2($que->date_shift, $que->date_shift, $que->userid,"Upacara");
					$og = $op->first_row();

                    if (isset($que->date_in)) {
                        //$date_in = date('d-m-Y', strtotime($que->date_in)). ' ' . $que->check_in;
                        if($og != '' or $og != null){
                            $date_in = date('d-m-Y H:i:s', strtotime($og->checktime));
                        }
                    } 
            ...
            foreach ($dataallay as $queqe) 
            {
                ...
                foreach ($querytemp->result() as $que)
                {
					$op = $this->report_model->gettranslog2($que->date_shift, $que->date_shift, $queqe['userid'],$moderpt);
					$og = $op->first_row();
