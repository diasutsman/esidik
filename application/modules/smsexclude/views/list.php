...
<div class="table-responsive">
    ...
    <table class="table table-striped  table-bordered">
        ...
        <tr>
            ...
            <th  onClick="load_url('<?php echo $url_pag?>','a.userid','<?php echo (isset($order)?(($order=='a.userid')?$sorting:'sorting'):'sorting_asc')?>')"><div class="<?php echo (isset($order)?(($order=='a.userid')?$typeorder:'sorting'):'sorting_asc')?>">NIP</div></th>
            <th  onClick="load_url('<?php echo $url_pag?>','b.name','<?php echo (isset($order)?(($order=='b.name')?$sorting:'sorting'):'sorting_asc')?>')"><div class="<?php echo (isset($order)?(($order=='b.name')?$typeorder:'sorting'):'sorting_asc')?>">Nama</div></th>
            <th></th>
        </tr>
        </thead>
        <tbody>
            ...
            <div class="col-sm-6 m-b-xs">
                <div id="tabel_data_length" class="dataTables_length">
                    <div class="form-inline">
                        ...
                        $js = 'id="limit_display" class="input-sm" onChange="load_url(\''.$url_pag.'\',\''.(isset($order)?$order:'a.userid').'\',\''.(isset($typeorder)?$typeorder:'sorting').'\');" name="tabel_data_length" size="1" ';
...
<script>
    $(function(){
        ...
        $('.editrow').click(function(){
            ...
            $.getJSON('<?php echo site_url('smsexclude/edit')?>/'+xno,function() {})
                .done(function(response) {
                    location.hash = "#inputForm";
                    $("#iddata").val(xno);
                    $('#txt1').val(response['userid']);
                    $("#btnAdd").hide();
                    $("#btnDeleteRow").hide();
    ...
    function createpaging()
    {
        $("#pagering").find("a").each(function(i){
            ...
            $(this).bind('click', function(){
                load_url(thisHref,'<?php echo (isset($order)?$order:'a.userid')?>','<?php echo (isset($typeorder)?$typeorder:'sorting')?>');
                return false;
            });
        });
    }
