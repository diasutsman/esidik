    ...
    <table class="table table-striped table-bordered">
        ...
        foreach($result as $row)
        {
            ...
            <tr id="rowdata-<?php echo $row->id?>">
                ...
                <td><?php echo strlen($row->nama_unit)>50 ? ellipsize($row->nama_unit, 50, .5):$row->nama_unit;?></td>
                <td><?php echo $row->nama_area;?></td>
                <td><?php echo $row->userid;?></td>
                <td><?php echo $row->state==1?"Aktif":"Non Aktif"?></td>
                <td width="80px"><a class="btn btn-xs btn-warning editrow" data-id="<?php echo $row->id?>"><i class="fa fa-pencil"></i></a>
                    &nbsp;<a class="btn btn-xs btn-danger resetrow" data-id="<?php echo $row->id?>" title="Reset Password"><i class="fa fa-undo"></i></a>
                </td>
            </tr>
            <?php
           ...
           /* $('#ukertree').jstree("deselect_all");
            ...
            $.getJSON('<?php echo site_url('usrdata/edit')?>/'+xno,function() {})
                .done(function(response) {
                    ...
                    var mySplitResult;
                    if (response['dept_id'] != null ) {
                        mySplitResult = response['dept_id'].split(',');
                        // $('#ukertree').jstree("select_node", response['dept_id'], false);
                        //$('#ukertree').jstree("check_node", response['dept_id']);
                        for (i = 0; i < mySplitResult.length; i++) {
                            $('#ukertree').jstree('check_node', '#' + mySplitResult[i]);
                        }
                        $("#txt4").val(response['dept_id']);
                    }

                   // $('#areatree').jstree("select_node", response['area_id'], false);
                    //$('#ukertree').jstree("check_node", response['area_id']);
                    if (response['area_id'] != null ) {
                        mySplitResult = response['area_id'].split(',');
                        for (i = 0; i < mySplitResult.length; i++) {
                            $('#areatree').jstree('check_node', '#' + mySplitResult[i]);
                        }
                    }
