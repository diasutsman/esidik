...
class Rptuangmakan extends MX_Controller {
    ...
    public function view()
    {
        ...
        $query = $this->pegawai->getDaftar(0,null,null,null,$SQLcari);
        ...
        foreach($query->result() as $row)
        {
            $kdGol = konversiGolongan($row->golru);
            $umk = $this->uangmakan->ref_uangmakan($kdGol);
            //$jml = $this->pegawai->getrealabsensi($row->userid,$startdate,$enddate);
            /*$jml = $this->pegawai->gethistabsensi($row->userid,$startdate,$enddate);
            $jmlkotor = $jml["jmlHari"] * $umk["nominal"];
            $pajak = $jmlkotor*($umk["pajak"]/100);
            $jmlBersih = $jmlkotor-$pajak;
            $datapeg[]=array("userid"=>$row->userid,"name"=>$row->name,"golru"=>$row->golru,
                "tarif"=>$umk["nominal"],"pajak"=>$pajak,"hadir"=>$jml["jmlHari"],
                "kotor"=>$jmlkotor,"bersih"=>$jmlBersih);*/

            $listdata = $this->pegawai->getlisthistabsensi($row->userid,$startdate,$enddate,$row->deptid);
