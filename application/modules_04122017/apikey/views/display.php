...
<script>
	$(document).ready(function(){
		...
		$('#btncari').click(function(){
			load_url('<?php echo site_url('apikey/pagging/0') ?>',"<?php echo (isset($order)?$order:'id')?>","<?php echo (isset($typeorder)?$typeorder:'sorting_desc')?>");
		});
		...
		if(cek > 0){
			bootbox.confirm("Anda yakin menghapus data yang dipilih?", function(result) {
				if(result) {
					...
					$.ajax({
						url     : '<?php echo site_url('harilbr/hapus');?>',
						dataType: 'json',
						type    : 'POST',
						data    : { 'id' : cek_del},
