...
class Setholy extends MX_Controller {
    private $aAkses;
	function Setholy(){
		parent::__construct();
		...
		$this->load->model('setholy_model','harilbr');

        $this->aAkses = akses("Setholy", $this->session->userdata('s_access'));
	...
	function index_list()
	{
        ...
        $this_url = site_url('setholy/pagging/');
    ...
    public function save()
    {
        ...
        if ($idx==0)
        {
            $insert = $this->db->insert('defa_holiday', $dataIn);
            if (!$insert) {
        ...
        else
        {
            ...
            $data=$this->db->get("defa_holiday")->row_array();
    ...
    public function hapus()
    {
        ...
        for($i=0;$i<count($id);$i++){
            ...
            $query=$this->db->get("defa_holiday");
            $datas = $query->row_array();
            log_history("delete","defa_holiday",$datas);

            $this->db->where('id',$nId);
            $this->db->delete('defa_holiday');
