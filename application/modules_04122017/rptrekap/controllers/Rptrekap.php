...
class Rptrekap extends MX_Controller {
    ...
    public function view2($issimple=1)
    {
        ...
        $sqlcok = "select a.userid, date_shift, check_in, check_out, attendance, workinholiday, late, early_departure
						...
						where ".$tambahan." and (date_shift >= '".$this->db->escape_str($start_date)."' and 
						date_shift <= '".$this->db->escape_str($end_date)."') ".$with_jns_pegawai.$with_sts_pegawai;
        ...
        if($simple==1) {
            $query_att_perdate = "SELECT userid, name, deptname, title, golru, kelasjabatan
                                    ...
                                    ORDER BY eselon, golru desc, kelasjabatan desc";
        } else {
