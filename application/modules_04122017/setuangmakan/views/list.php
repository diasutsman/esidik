    ...
    <div class="dataTables_wrapper dt-bootstrap">
        <table class="table table-striped  table-bordered">
            ...
            <tr>
                ...
                <th onClick="load_url('<?php echo $url_pag?>','golongan','<?php echo (isset($order)?(($order=='golongan')?$sorting:'sorting'):'sorting')?>')" ><div class="<?php echo (isset($order)?(($order=='golongan')?$typeorder:'sorting'):'sorting')?>">Golongan</div></th>
                <th onClick="load_url('<?php echo $url_pag?>','nominal','<?php echo (isset($order)?(($order=='nominal')?$sorting:'sorting'):'sorting')?>')" ><div class="<?php echo (isset($order)?(($order=='nominal')?$typeorder:'sorting'):'sorting')?>">Tarif</div></th>
                <th onClick="load_url('<?php echo $url_pag?>','pajak','<?php echo (isset($order)?(($order=='pajak')?$sorting:'sorting'):'sorting')?>')" ><div class="<?php echo (isset($order)?(($order=='pajak')?$typeorder:'sorting'):'sorting')?>">Pajak</div></th>
            ...
            foreach($result as $row)
            {
                ...
                <tr id="rowdata-<?php echo $row->id?>">
                    ...
                    <td><?php echo $this->utils->getRefGolongan()[$row->golongan];?></td>
                    <td><?php echo format_angka($row->nominal);?></td>
                    <td><?php echo format_angka($row->pajak);?></td>
...
<script>
    $(function(){
        ...
        $('.editrow').click(function(){
            ...
            $.getJSON('<?php echo site_url('setuangmakan/edit')?>/'+xno,function() {})
                .done(function(response) {
                    location.hash = "#inputForm";
                    $("#iddata").val(xno);
                    $('#txt1').val(response['golongan']);
                    $("#txt2").val(response['nominal']);
                    $("#txt3").val(response['pajak']);
