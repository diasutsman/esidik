...
class Mtunkir extends MX_Controller {
	...
	function Mtunkir(){
        ...
        $this->aAkses = akses("Mtunkir", $this->session->userdata('s_access'));
    ...
    {
        $arr = $this->db->get_where('mastertunjangan', array('kelasjabatan' => $noId))->row_array();
        $arr["tmt"] = ymdTodmy($arr["tmt_berlaku"]);
    ...
    public function save()
    {
        ...
        $t3 = $this->input->post('tmt');

        $dataIn['kelasjabatan'] = $t1;
        $dataIn['tunjangan'] =  $t2;
        $dataIn['tmt_berlaku'] =  dmyToymd($t3);
