	...
	<div class="popup-wrapper fade" id="popup">
		<div class="popup-container">
			<div class="modal-header">
				...
				<h3>Input Jadwal Khusus</h3>
			</div>
			<div class="modal-body" id="input_data" style="max-height:450px;min-height:250px;">
...
<script>
	$(function(){
		...
		$('#btncari').click(function(){
			load_url('<?php echo site_url('jadwalkhusus/pagging/0') ?>', '<?php echo (isset($order)?$order:'id')?>','<?php echo (isset($typeorder)?$typeorder:'sorting')?>');
		});
	...
	function doAdd()
	{
		...
		var url ="<?php echo site_url('jadwalkhusus/form')?>";
		...
		if(cek > 0){
			bootbox.confirm("Anda yakin menghapus data yang dipilih?", function(result) {
				if(result) {
					...
					$.ajax({
						url     : '<?php echo site_url('jadwalkhusus/hapus');?>',
						dataType: 'json',
						type    : 'POST',
						data    : { 'id' : cek_del},
						success : function(data){
	...
	function saveForm() {
		if (($("#txt1").val() != "")) {
			$.ajax({
				url: '<?php echo site_url('jadwalkhusus/save');?>', dataType: 'json', type: 'POST', data: $("#inputForm").serialize(), success: function (data) {
					bootbox.alert(data.msg);
				}
			});
		} else {
