			...
			<div class="row form-horizontal">
				...
				<div class="form-group">
					<div class="col-md-10 col-md-offset-2">
                        <button type="button" id="btncaridata" class="btn btn-sm btn-primary"> Display</button> <button type="button" id="btnproses" class="btn btn-sm btn-success"><i class="fa fa-user"></i> Proses Pegawai</button> <button type="button" id="btnsemua" class="btn btn-sm btn-warning"><i class="fa fa-users"></i>  Proses Semua</button>
					</div>
				</div>
			</div>
		</div>
...
<script>
	$(function(){
		...
		$('#btncaridata').click(function(){
			load_url('<?php echo site_url('repost/pagging/0') ?>', '<?php echo (isset($order)?$order:'id')?>','<?php echo (isset($typeorder)?$typeorder:'sorting')?>');
		});
        ...
        $('#btnproses').click(function(){
            ...
            if(cek > 0)
            {
                ...
                $.ajax({
                    dataType: 'json',
                    type    : 'POST',
                    url: '<?php echo site_url('repost/pegawai') ?>',
                    data:{"org":par6,"stspeg":par7,"userid":cek_del,"startdate":par8,"enddate":par9},
                    success: function(data){
            ...
            bootbox.confirm("Anda yakin memproses semua data (berdasarkan unit kerja)?", function(result) {
                if(result) {
                    ...
                    $.ajax({
                        dataType: 'json',
                        type    : 'POST',
                        url: '<?php echo site_url('repost/allpegawai') ?>',
                        data:{"org":par6,"stspeg":par7,"startdate":par8,"enddate":par9},
                        success: function(data){
                            bootbox.alert(data.msg);
                        },
                        beforeSend: function() {
