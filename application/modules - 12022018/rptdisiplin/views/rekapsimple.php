<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');?>

<html xmlns="http://www.w3.org/1999/xhtml">
<head>
    <style>
        <?php include FCPATH."/assets/css/print.css"; ?>
    </style>
	<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<title><?php echo $this->lang->line('menu_main_recapreport'); ?></title>
</head>

<body>
<style> .num {
        mso-number-format:General;
    }
    .text{
        mso-number-format:"\@";/*force text*/
    }
</style>
<?php 

$page=1;
$headerrep = '<table class="head">
	<tr>
    	<td width="180" rowspan="4" align="center" style="border:0px">';
if($cominfo['logo']!='') {
	$headerrep .= '<img src="'.base_url().$cominfo['logo'].'" width="50" />';
	$clss = 'align="left"'; 
} else { 
	$clss = 'align="center"'; 
} 

$headerrep .= '</td>
    	<td width="10" rowspan="4" style="border:0px">&nbsp;</td>
        <td '.$clss.' style="border:0px" class="text"><h1>'.$cominfo['companyname'].'</h1></td>
    </tr>
	<tr>
    	<td '.$clss.' style="border:0px" class="text">'.$cominfo['address1'].'</td>
    </tr>';

	if ($cominfo['address2']!='') {
		$headerrep .= '
    <tr>
    	<td '.$clss.' style="border:0px"  class="text">'.$cominfo['address2'].'</td>
    </tr>';
	}
	$headerrep .= '
    <tr>
      <td '.$clss.' style="border:0px" class="text">'.$this->lang->line('phone').': '.$cominfo['phone'].', '.$this->lang->line('fax').': '.$cominfo['fax'].'</td>
	  <td width="10" rowspan="4" style="border:0px">&nbsp;</td>
	  <td width="180" rowspan="4" align="center" style="border:0px">&nbsp;</td>
    </tr>
</table>
<hr />';
$atco = count($att); $abco = count($abs); 
$headcol ='<thead> <tr>
     <th rowspan="2" align="center" class="text">No</th>
     <th rowspan="2" align="center" class="text">'.$this->lang->line('empidrecap').'</th>
    <th rowspan="2" align="center" class="text">'.$this->lang->line('empnamerecap').'</th>    
    <th rowspan="2"  align="center" class="text">Kehadiran</th> 	
    <th rowspan="2" align="center" class="text">Tidak Hadir</th>
    <th colspan="'.$abco.'" align="center" class="text">Status Kerja</th>
  </tr>
  <tr>';
    
		foreach($abs as $abse) {
			$headcol .='<th align="center">'.$abse['abid'].'</th>';
		}
	
$headcol .=' </tr></thead>';

	$height = 0;
$clss='';
	if($cominfo['companyname']!='') {
	echo $headerrep;
	} ?>
<h1><center>
    <?php echo strtoupper($this->lang->line('menu_main_recapreport')); ?>
</center></h1><br>
<table class="head">
	<tr>
    	<td width="120" style="border:0px" class="text"><?php echo $this->lang->line('menu_main_organization'); ?></td>
        <td style="border:0px" class="text">: <?php echo $empinfo['dept']; ?></td>
        <td width="120" style="border:0px" class="text"><?php echo $this->lang->line('totholiday'); ?></td>
        <td style="border:0px" class="text">: <?php echo $empinfo['holidays']; ?></td>
        <td width="40%" style="border:0px">&nbsp;</td>
    </tr>
	<tr>
    	<td width="120" style="border:0px" class="text"><?php echo $this->lang->line('periode'); ?></td>
        <td style="border:0px" class="text">: <?php echo $empinfo['datestart'].' - '.$empinfo['datestop']; ?></td>
        <td width="120" style="border:0px">&nbsp;</td>
        <td style="border:0px">&nbsp;</td>
        <td width="40%" style="border:0px">&nbsp;</td>
    </tr>
</table><br>

<table width="100%" border="1" cellspacing="0" cellpadding="0">
  
  <?php 
	$height += 80;
	echo $headcol."<tbody>";
	$x=1;
	foreach($data as $row) { 
		$height += 10;
  ?>
  <tr class="<?php if($x % 2 === 0) echo 'even'; else echo 'odd'; ?>">
    <td class="text"><?php echo $x; ?></td>
    <td class="text"><?php echo "&nbsp;".$row['badgeNumber']; ?></td>
    <td class="text"><?php echo $row['name']; ?></td>
    <td align="center" class="text"><?php echo $row['workday']; ?></td>
    <td align="center" class="text"><?php echo $row['absent']; ?></td>
    <?php 
		foreach($abs as $abse) {
			if($row['aben'][$row['userid']][$abse['abid']]!=0) {
				echo '<td align="center" class="text">'.$row['aben'][$row['userid']][$abse['abid']].'</td>';
			} else {
				echo '<td align="center" class="text">-</td>';
			}
		}
	?>
   
  </tr>
  <?php 
	//echo $height.'====';
	/*if($page==1){
		if($height%500==0 ){
			echo "</table><br  style='page-break-before: always;'>";
			echo "<table width='100%' border='1' cellspacing='0' cellpadding='0'>";
			echo $headcol;
			$page++;
		} 
	}else{
		if($height%600==0 ){
			echo "</table><br  style='page-break-before: always;'>";		
			echo "<table width='100%' border='1' cellspacing='0' cellpadding='0'>";
			echo $headcol;
			$page++;
		} 		
	}*/	
	$x++; } ?>
</tbody></table><br><br>
<table class="head">
	<tr>
    	<td width="50px" style="border:0px">Kepala Subbagian Tatalaksana <br>dan Kepegawaian,</td>        
    </tr>
	<tr><td width="50px" style="border:0px"></td></tr>
	<tr><td width="50px" style="border:0px"></td></tr>
	<tr><td width="50px" style="border:0px"></td></tr>
	<tr><td width="50px" style="border:0px"></td></tr>
	<tr><td width="50px" style="border:0px"></td></tr>
	<tr><td width="50px" style="border:0px"></td></tr>
	<tr>
    	<td width="50px" style="border:0px">Suharyoko, S.H.</td>        
    </tr>
   
</table>