...
class Presensi extends Api_Controller {
	...
	public function sikerja_post()
    {
        ...
        if ($rwo->num_rows()>0) {
            ...
            $arrint= array(
                'token_api'=>$token,
                'api_method'=>'sikerja_post',
                'api_ip'=>getRealIpAddr()
            );

            $this->db->insert('api_log',$arrint) ;
            $hasil = $this->view1($nip,$bulan);
            $message = array(
                'status' => TRUE, // Automatically generated by the model
                'results' => $hasil
            );
        ...
        foreach($dataallay as $queqe) {
            ...
            foreach($querytemp->result() as $que) {
                ...
                if($krglate > 0 && $que->attendance!='NWK' &&
                    ...
                    $que->workinholiday!=1 ) {
                    $dataarray[] = array(
                        'tanggal'			=> date('d-m-Y', strtotime($que->date_shift)),
            ...
            $dataallaye = array(
                ...
                'satuan_kerja' => $queqe['deptName'],
                'kelas_jabatan' => $queqe['kelasjabatan'],
                // 'tunjangan' => $tottun,
                // 'tunjangan_plt' => $totplt,
                // 'plt_jbtn' => $queqe['plt_jbtn'],
                // 'plt_kelasjabatan' => $queqe['plt_kelasjabatan'],
                'isplt' => $queqe['isplt'],
                // 'satker_plt' => $queqe['plt_deptname'],
                // 'kriteria_plt' => $queqe['kriteriaPlt'],
            );

            $datafoot = array('total_persen' => $totalpersensemua,
                'total' => $tottun == 0 ? 0 : $tunj,
                'total_plt' => $totplt == 0 ? 0 : $tunjplt
            );

            $byuser= $userid!='undefined'?1:0;

            $data = array(
                "infopegawai" => $dataallaye,
                //"rekap" => $datafoot,
                "data" => $dataarray,
                // "totaltunj" =>$totaltunj,
                // "totaltunj_plt" => $totaltunjplt,
                // "byuser"=>$byuser,
