...
class Prosesdata extends CI_Controller
{
	...
	public function allpegawai()
    {
       ...
       /* $this->db->select('status');
        ...
        //if($bukatutup)
        {
            ...
            //$orgid = $this->pegawai->deptonall($orgidi);
            $range = ($dateend - $datestart) / 86400;
            $shift = $this->process_model->getshiftallorg($datestart, $dateend);
            $arrai = array();
            foreach($shift as $shifting) {
            ...
            $user = $this->process_model->getuserbyallorg();
            foreach($user->result() as $userya) {
                $useraidi[] = $userya->userid;
                $deptid[$userya->userid] = $userya->deptid;
            }
                ...
                foreach($useraidi as $userdata) {
                    for($x=0;$x<=$range;$x++) {
                        ...
                        if(isset($arrai[$userdata][strtotime($tanggal)])) {
                            ...
                            if( $arrai[$userdata][strtotime($tanggal)]['codeshift'] == 'PMD'  ||
                                $arrai[$userdata][strtotime($tanggal)]['codeshift'] == 'S1C') {
                                $tanggalsco = date('Y-m-d', strtotime($tanggal) + 86400);
                                $tanggalco = date('Y-m-d', strtotime($tanggal) + 86400);
                                $tanggaleco = date('Y-m-d', strtotime($tanggal) + 86400);
	...
	public function dashboarddata()
    {
        ...
        $fileJson='data_'.date("Y-m-d").'.json';
        //delete all file except today
        $dir = 'assets/dashboard';
        $leave_files = array($fileJson);
        ...
        $fp = fopen('assets/dashboard/'.$fileJson, 'w');
                ...
                foreach($querytemp->result() as $que) {
                    ...
                    if(isset($tunbul[date('mY', strtotime($que->date_shift))])) {
                        if(isset($tunbul[date('mY', strtotime($que->date_shift))][date('j', strtotime($que->date_shift))])) {
                            $tunjangan[date('mY', strtotime($que->date_shift))] = $ttlmsk[date('mY', strtotime($que->date_shift))] == 0 ? 0 :$tunbul[date('mY', strtotime($que->date_shift))][date('j', strtotime($que->date_shift))] / $ttlmsk[date('mY', strtotime($que->date_shift))];
                            ...
                            $tunjanganplt[date('mY', strtotime($que->date_shift))] = $ttlmsk[date('mY', strtotime($que->date_shift))]==0 ? 0 :$tunbulplt[date('mY', strtotime($que->date_shift))][date('j', strtotime($que->date_shift))] / $ttlmsk[date('mY', strtotime($que->date_shift))];
