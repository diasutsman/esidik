...
$url_pag =  site_url("statabsen/pagging/0");
$sorting='';
...
<div class="table-responsive">
    ...
    <table class="table table-striped table-bordered">
        ...
        <tr>
            ...
            <th onClick="load_url('<?php echo $url_pag?>','abid','<?php echo (isset($order)?(($order=='abid')?$sorting:'sorting'):'sorting')?>')" ><div class="<?php echo (isset($order)?(($order=='abid')?$typeorder:'sorting'):'sorting')?>">Kode</div></th>
            <th onClick="load_url('<?php echo $url_pag?>','abname','<?php echo (isset($order)?(($order=='abname')?$sorting:'sorting'):'sorting')?>')"><div class="<?php echo (isset($order)?(($order=='abname')?$typeorder:'sorting'):'sorting')?>">Keterangan</div></th>
        ...
        foreach($result as $row)
        {
            ...
            <tr id="rowdata-<?php echo $row->id?>">
                ...
                <td><?php echo $row->abid;?></td>
                <td><?php echo $row->abname;?></td>
...
<script>
    $(function(){
        ...
        $('.editrow').click(function(){
            ...
            $.getJSON('<?php echo site_url('statabsen/edit')?>/'+xno,function() {})
            .done(function(response) {
                location.hash = "#inputForm";
                $("#iddata").val(xno);
                $('#txt1').val(response['abid']);
                $("#txt2").val(response['abname']);
