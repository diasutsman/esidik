...
<?php
	$sql = $this->db->query("SELECT * FROM personnel_area WHERE parent_id=$param ORDER BY areaid ASC");
	foreach($sql->result() as $row){

		$sql2 = $this->db->query("select * from personnel_area WHERE parent_id=".$row->areaid." ORDER BY areaid ASC");
		if($sql2->num_rows() > 0){
			?>
				<div class="tree-folder" style="display: block;">
					<div class="tree-folder-header hd<?php echo $lvl?>">
						<i id="fd<?php echo $row->areaid?>" class="icon-plus" onClick="TreeMenu('<?php echo $row->areaid?>',<?php echo $lvl+1?>)"></i>
						<div class="tree-folder-name fn<?php echo $lvl?>" onClick="PilihData('<?php echo $row->areaid?>','<?php echo $row->areaname?>');"><?php echo $row->areaname?></div>
						</div>
		...
		<?php
			...
			<div class="tree-item hd<?php echo $row->areaid?>" style="display: block;" onClick="PilihData('<?php echo $row->areaid?>','<?php echo $row->areaname?>');">
				<div class="tree-item-name fn<?php echo $row->areaid?>"><?php echo $row->areaname?></div>
			</div>
		<?php
		}
	}
