...
$url_pag =  site_url("jadwalkrj/pagging/0");
    ...
    <div class="dataTables_wrapper dt-bootstrap">
        <table class="table table-bordered small" id="tbl">
            ...
            <tr>
                ...
                <th rowspan="2" onClick="load_url('<?php echo $url_pag?>','name','<?php echo (isset($order)?(($order=='name')?$sorting:'sorting'):'sorting')?>')"><div class="<?php echo (isset($order)?(($order=='name')?$typeorder:'sorting'):'sorting')?>">Nama</div></th>
                <?php
                $str1=$start_date;
                $end1=$end_date;
                $xxi=1;
                ...
                $end1=$end_date;
                while (strtotime($str1) <= strtotime($end1)) {
                    echo "<th class='text-center' data-original-title='".date("Y-m",strtotime($str1))."' data-toggle='tooltip' data-placement='top' data-container='body'>".date("j",strtotime($str1))."</th>";
                    $str1 = date("Y-m-d", strtotime("+1 days", strtotime($str1)));
                }
                ?>
            </thead>
                ...
                //print_r($result);
                    foreach ($result as $key =>$value)
                    {
                        ...
                        <tr>
                            ...
                            <td id="<?php echo $userid.'-name'?>" data-tgl='' data-fid="name"  data-id="<?php echo $userid?>"  data-rec="<?php echo $value["name"]?>"  class="rows"><?php echo $value["name"]?></td>
                            <?php
                            ...
                            while (strtotime($str2) <= strtotime($end2)) {
                                ...
                                if (array_key_exists(strtotime($str2),$value))
                                {
                                    ...
                                    echo "<td style='background-color:$col' class='rows text-center' id='".$userid.'-'.$str2."'data-fid='$str2' data-id='$userid' data-tgl='$lbltgl' data-rec='".$lrec ."'>" .$lket."</td>";
                                } else
                                {
                                    echo "<td style='background-color:$col' class='rows text-center' id='".$userid.'-'.$str2."'data-fid='$str2' data-id='$userid' data-tgl='$lbltgl' data-rec='".$lrec ."'>" .$lket."</td>";
                                }
...
<script>
    ...
    $(function(){
        ...
        $(".rows").click(function (){
            ...
            $.ajax({
                method:"post",
                url: "<?php echo site_url("jadwalkrj/gethistory")?>",
        ...
        /*$('.editrow').click(function(){
            ...
            $.getJSON('echo site_url('jadwalkrj/edit')'+xno,function() {})
